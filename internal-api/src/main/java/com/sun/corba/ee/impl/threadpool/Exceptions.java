...
/**
 * Exception wrapper class. The logex WrapperGenerator uses this interface to generate an implementation which returns
 * the appropriate exception, and generates a log report when the method is called. This is used for all implementation
 * classes in this package.
 *
 * The exception IDs are allocated in blocks of EXCEPTIONS_PER_CLASS, which is a lot more than is needed, but we have 32
 * bits for IDs, and multiples of a suitably chosen EXCEPTIONS_PER_CLASS (like 100 here) are easy to read in error
 * messages.
 *
 * @author ken
 */
...
public interface Exceptions {
    static final Exceptions self = WrapperGenerator.makeWrapper(Exceptions.class, StandardLogger.self);
...
// ThreadPoolImpl
    ...
    @Log(id = TP_START + 0)
    void interruptedJoinCallWhileClosingThreadPool(@Chain InterruptedException exc, Thread wt, ThreadPoolImpl aThis);

    @Message("Worker Thread {0} has been created with ClassLoader {1}")
    @Log(id = TP_START + 0, level = LogLevel.FINE)
    void workerThreadCreated(Thread thread, ClassLoader contextClassLoader);
    ...
    @Log(id = TP_START + 2)
    RuntimeException workerThreadGetContextClassloaderFailed(@Chain SecurityException se, Thread aThis);
    ...
    void workerThreadForgotClassloaderReset(Thread aThis, ClassLoader currentClassLoader, ClassLoader workerThreadClassLoader);

    @Message("Unable to set worker thread {0}; check securiy policy file:" + " must grant 'setContextClassLoader' runtime permission")
    @Log(id = TP_START + 5)
    void workerThreadResetContextClassloaderFailed(@Chain SecurityException se, Thread aThis);
    ...
    void workerThreadNotNeeded(Thread aThis, int currentNumberOfThreads, int minimumNumberOfThreads);

    @Message("Worker thread from thread pool {0} was interrupted:" + " closeCalled is {1}")
    @Log(id = TP_START + 8, level = LogLevel.FINE)
    void workQueueThreadInterrupted(InterruptedException exc, String name, Boolean valueOf);

    @Message("Worker thread {0} caught throwable when" + " requesting work from work queue {1}")
    @Log(id = TP_START + 9, level = LogLevel.FINE)
    void workerThreadThrowableFromRequestWork(@Chain Throwable t, Thread aThis, String name);

    @Message("Worker thread {0} caught unexpected throwable")
    @Log(id = TP_START + 10)
    void workerThreadCaughtUnexpectedThrowable(@Chain Throwable e, Thread aThis);
...
// ThreadPoolManagerImpl
    ...
    @Log(id = TPM_START + 2)
    void threadGroupHasActiveThreadsInClose(ThreadGroup thgrp, int numThreads);

    @Message("ThreadGroup {0} has {1} sub-ThreadGroups: destroy may cause exceptions")
    @Log(id = TPM_START + 3)
    void threadGroupHasSubGroupsInClose(ThreadGroup threadGroup, int numGroups);

    @Message("ThreadGroup {0} could not be destroyed")
    @Log(id = TPM_START + 4)
    void threadGroupDestroyFailed(@Chain IllegalThreadStateException exc, ThreadGroup threadGroup);
