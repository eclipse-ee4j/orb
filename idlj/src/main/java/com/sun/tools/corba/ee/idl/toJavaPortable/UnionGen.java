...
public class UnionGen implements com.sun.tools.corba.ee.idl.UnionGen, com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator {
    ...
    protected void writeBody() {
        ...
        if (u.defaultBranch() == null && !coversAll(u)) {
            ...
            stream.println();
            stream.println("  public void _default (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.javaName(utype) + " discriminator)");
            stream.println("  {");
    ...
    private void writeVerifyDefault() {
        ...
        stream.println("");
        stream.println("  private void verifyDefault( " + com.sun.tools.corba.ee.idl.toJavaPortable.Util.javaName(utype) + " value )");
        stream.println("  {");
    ...
    private void writeBranchMethods(PrintWriter stream, UnionEntry u, UnionBranch branch, int i) {
        ...
        stream.println();
        // <f46082.51> Remove -stateful feature; javaStatefulName() obsolete.
        // stream.println (" public void " + branch.typedef.name () + " (" + Util.javaStatefulName (branch.typedef) + "
        // value)");
        ...
        if (branch.labels.size() == 0) {
            ...
            if (unionIsEnum)
                stream.println("    __discriminator = " + typePackage
                        + com.sun.tools.corba.ee.idl.toJavaPortable.Util.parseExpression((Expression) branch.labels.firstElement()) + ";");
            else
        ...
        if (branch.labels.size() > 0 || branch.isDefault) {
            stream.println();
            // <f46082.51> Remove -stateful feature; javaStatefulName() obsolete.
            // stream.println (" public void " + branch.typedef.name () + " (" + Util.javaName (utype) + " discriminator, " +
            // Util.javaStatefulName (branch.typedef) + " value)");
    ...
    public int helperType(int index, String indent, com.sun.tools.corba.ee.idl.toJavaPortable.TCOffsets tcoffsets, String name, SymtabEntry entry,
        ...
        index = ((com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator) u.type().generator()).type(index + 1, indent, innerOffsets, discTypeCode, u.type(),
                stream);
        tcoffsets.bumpCurrentOffset(innerOffsets.currentOffset());

        stream.println(indent + "org.omg.CORBA.UnionMember[] " + membersName + " = new org.omg.CORBA.UnionMember [" + unionLabelSize(u) + "];");
            ...
            if (labels.size() == 0) {
                ...
                while (enumeration.hasMoreElements()) {
                    ...
                    if (utype instanceof PrimitiveEntry)
                        stream.println(indent + anyOfMembers + ".insert_" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.collapseName(utype.name()) + " (("
                                + com.sun.tools.corba.ee.idl.toJavaPortable.Util.javaName(utype) + ')' + elem + ");");
                    else { // it must be enum
                        String enumClass = com.sun.tools.corba.ee.idl.toJavaPortable.Util.javaName(utype);
                        stream.println(indent + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(utype, false) + ".insert (" + anyOfMembers + ", "
                                + enumClass + '.' + elem + ");"); // <d61056>
                    }
        ...
        stream.println(indent + name + " = org.omg.CORBA.ORB.init ().create_union_tc (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(u, true)
                + ".id (), \"" + entry.name() + "\", " + discTypeCode + ", " + membersName + ");");
        return index;
    } // helperType
    ...
    private int readBoolean(String disName, int index, String indent, String name, UnionEntry u, PrintWriter stream) {
        ...
        if (noCases) {
            ...
            if (firstBranch == null)
                ...
                index = readBranch(index, indent + "  ", firstBranch.typedef.name(), disName, firstBranch.typedef, stream);
                stream.println(indent + '}');
            }
            ...
            if (secondBranch == null)
                ...
                stream.println(indent + '{');
                index = readBranch(index, indent + "  ", secondBranch.typedef.name(), disName, secondBranch.typedef, stream);
                stream.println(indent + '}');
            }
        }
    ...
    private int readNonBoolean(String disName, int index, String indent, String name, UnionEntry u, PrintWriter stream) {
        ...
        while (e.hasMoreElements()) {
            ...
            if (!branch.typedef.equals(u.defaultBranch())) {
                index = readBranch(index, indent + "    ", branch.typedef.name(), branch.labels.size() > 1 ? disName : "", branch.typedef, stream);
        ...
        if (!coversAll(u)) {
            ...
            if (u.defaultBranch() == null) {
                ...
                index = readBranch(index, indent + "    ", u.defaultBranch().name(), disName, u.defaultBranch(), stream);
            }
    ...
    public int write(int index, String indent, String name, SymtabEntry entry, PrintWriter stream) {
        ...
        if (utype instanceof PrimitiveEntry)
            ...
            stream.println(
                    indent + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(utype, true) + ".write (ostream, " + name + ".discriminator ());"); // <d61056>
        if (utype.name().equals("boolean"))
    ...
    private int writeBranch(int index, String indent, String name, TypedefEntry entry, PrintWriter stream) {
        ...
        if (!entry.arrayInfo().isEmpty() || type instanceof SequenceEntry || type instanceof PrimitiveEntry || type instanceof StringEntry)
            ...
            stream.println(
                    indent + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(type, true) + ".write (ostream, " + name + '.' + entry.name() + " ());"); // <d61056>
        return index;
    } // writeBranch
