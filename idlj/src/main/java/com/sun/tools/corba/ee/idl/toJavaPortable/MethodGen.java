...
public class MethodGen implements com.sun.tools.corba.ee.idl.MethodGen {
    ...
    protected void stub(String className, boolean isAbstract, Hashtable symbolTable, MethodEntry m, PrintWriter stream, int index) {
        localOptimization = ((Arguments) com.sun.tools.corba.ee.idl.toJavaPortable.Compile.compiler.arguments).LocalOptimization;
    ...
    protected void writeStubBody(String className) {
        ...
        stream.println(THREE_INDENT + "org.omg.CORBA.portable.InputStream $in = null;");
        stream.println(THREE_INDENT + "try {");
        stream.println(FOUR_INDENT + "org.omg.CORBA.portable.OutputStream $out =" + " _request (\"" + methodName + "\", " + !m.oneway() + ");");
        ...
        while (parms.hasMoreElements()) {
            ...
            if (parmType instanceof StringEntry)
                if ((parm.passType() == ParameterEntry.In) || (parm.passType() == ParameterEntry.Inout)) {
                    ...
                    if (string.maxSize() != null) {
                        ...
                        if (parm.passType() == ParameterEntry.Inout)
                            stream.print(".value"); // get from holder
                        stream.println(".length () > (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.parseExpression(string.maxSize()) + "))");
                        stream.println(THREE_INDENT + "throw new org.omg.CORBA.BAD_PARAM (0," + " org.omg.CORBA.CompletionStatus.COMPLETED_NO);");
                    }
                }
        }
        ...
        while (parms.hasMoreElements()) {
            ...
            if (parm.passType() == ParameterEntry.In)
                writeOutputStreamWrite(FOUR_INDENT, "$out", parm.name(), parm.type(), stream);
            else if (parm.passType() == ParameterEntry.Inout)
                writeOutputStreamWrite(FOUR_INDENT, "$out", parm.name() + ".value", parm.type(), stream);
        }

        // Step 2a. Write the context parameter if necessary. <d59560>
        if (m.contexts().size() > 0) {
            stream.println(FOUR_INDENT + "org.omg.CORBA.ContextList $contextList =" + "_orb ().create_context_list ();");

            for (int cnt = 0; cnt < m.contexts().size(); cnt++) {
                stream.println(FOUR_INDENT + "$contextList.add (\"" + m.contexts().elementAt(cnt) + "\");");
            }
            stream.println(FOUR_INDENT + "$out.write_Context ($context, $contextList);");
        }
        ...
        if (mtype != null)
            com.sun.tools.corba.ee.idl.toJavaPortable.Util.writeInitializer(FOUR_INDENT, "$result", "", mtype, writeInputStreamRead("$in", mtype), stream);
        ...
        while (parms.hasMoreElements()) {
            ...
            if (parm.passType() != ParameterEntry.In) {
                if (parm.type() instanceof ValueBoxEntry) {
                    ...
                    SymtabEntry mType = member.type();
                    if (mType instanceof PrimitiveEntry)
                        stream.println(FOUR_INDENT + parm.name() + ".value = (" + writeInputStreamRead("$in", parm.type()) + ").value;");
                    else
                        stream.println(FOUR_INDENT + parm.name() + ".value = " + writeInputStreamRead("$in", parm.type()) + ";");
                } else
                    stream.println(FOUR_INDENT + parm.name() + ".value = " + writeInputStreamRead("$in", parm.type()) + ";");
            }
        }
        ...
        while (parms.hasMoreElements()) {
            ...
            SymtabEntry parmType = com.sun.tools.corba.ee.idl.toJavaPortable.Util.typeOf(parm.type());
            if (parmType instanceof StringEntry)
                if ((parm.passType() == ParameterEntry.Out) || (parm.passType() == ParameterEntry.Inout)) {
                    StringEntry string = (StringEntry) parmType;
                    if (string.maxSize() != null) {
                        stream.print(FOUR_INDENT + "if (" + parm.name() + ".value.length ()");
                        stream.println("         > (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.parseExpression(string.maxSize()) + "))");
                        stream.println(FIVE_INDENT + "throw new org.omg.CORBA.MARSHAL(0," + "org.omg.CORBA.CompletionStatus.COMPLETED_NO);");
                    }
                }
        }
        if (mtype instanceof StringEntry) {
            ...
            if (string.maxSize() != null) {
                stream.println(
                        FOUR_INDENT + "if ($result.length () > (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.parseExpression(string.maxSize()) + "))");
                stream.println(FIVE_INDENT + "throw new org.omg.CORBA.MARSHAL (0," + " org.omg.CORBA.CompletionStatus.COMPLETED_NO);");
            }
        }
        ...
        stream.println(THREE_INDENT + "} catch (org.omg.CORBA.portable.ApplicationException " + "$ex) {");
        ...
        if (m.exceptions().size() > 0) {
            ...
            while (exceptions.hasMoreElements()) {
                ...
                stream.println(FIVE_INDENT + "throw " + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName((SymtabEntry) exc, false) + ".read ($in);");
            }
        ...
        stream.println(THREE_INDENT + "} catch (org.omg.CORBA.portable.RemarshalException $rm) {");
    ...
    /**
     * This method writes the else part of the stub method invocation to enable local invocation in case of collocation.
    ...
    private void writeStubBodyForLocalInvocation(String className, String methodName) {
        ...
        stream.println(THREE_INDENT + "org.omg.CORBA.portable.ServantObject _so =");
        stream.println(FOUR_INDENT + "_servant_preinvoke(\"" + methodName + "\", _opsClass);");
        ...
        if (!voidReturnType) {
            stream.println(FOUR_INDENT + com.sun.tools.corba.ee.idl.toJavaPortable.Util.javaName(this.m.type()) + " $result;");
        }
        if (!isValueInitializer()) {
            if (voidReturnType) {
                stream.print(FOUR_INDENT + "_self." + methodName + "( ");
            } else {
                stream.print(FOUR_INDENT + "$result = _self." + methodName + "( ");
            }
            while (parms.hasMoreElements()) {
    ...
    protected void writeLocalStubBody(InterfaceEntry i) {
        ...
        stream.println("      org.omg.CORBA.portable.ServantObject $so = " + "_servant_preinvoke (\"" + methodName + "\", " + "_opsClass);");
    ...
    /**
     * This is only used by AttributeGen. The java mapping says the names should be getXXX and setXXX, but CORBA says they
     * should be _get_XXX and _set_XXX. this.name () will be getXXX. realName is set by AttributeGen to _get_XXX.
     **/
    ...
    private void writeOutputStreamWrite(String indent, String oStream, String name, SymtabEntry type, PrintWriter stream) {
        ...
        if (type instanceof PrimitiveEntry) {
            ...
            if (mType instanceof PrimitiveEntry && name.endsWith(".value"))
                ...
                stream.println(com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(type, true) + ".write (" + oStream + ", " + name + ");"); // <d60929>
                                                                                                                                                   // //
                                                                                                                                                   // <d61056>
