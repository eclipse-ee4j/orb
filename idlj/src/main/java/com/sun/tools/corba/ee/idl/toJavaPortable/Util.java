...
/**
 * Class Util is a repository of static members available for general use by the IDL parser framework and any generator
 * extensions.
 **/
public class Util extends com.sun.tools.corba.ee.idl.Util {
    ...
    public static GenFileStream getStream(String name, SymtabEntry entry) {
        // <f46838.4>
        String absPathName = ((com.sun.tools.corba.ee.idl.toJavaPortable.Arguments) com.sun.tools.corba.ee.idl.toJavaPortable.Compile.compiler.arguments).targetDir
                + name;
    ...
    private static String doContainerFullName(SymtabEntry container) {
        ...
        if (container == null)
            ...
            if (container instanceof InterfaceEntry || container instanceof StructEntry || container instanceof UnionEntry)
    ...
    /**
     * Given a SymtabEntry, return the string which should be used for this entry. Enums are converted to ints, typedefs and
     * sequences are converted to their info types. javaQualifiedName does not do any of these conversions.
     **/
    ...
    static final short TypeFile = 0, StubFile = 1, HelperFile = 2, HolderFile = 3, StateFile = 4;
    ...
    public static void writePackage(PrintWriter stream, SymtabEntry entry, short type) {
        ...
        if (fullName != null && !fullName.equals("")) {
            ...
            /*
             * Enumeration e = Compile.compiler.importTypes.elements (); while (e.hasMoreElements ()) { SymtabEntry i =
             * (SymtabEntry)e.nextElement (); // Write import for type if (!(i instanceof TypedefEntry)) stream.println ("import " +
             * i.name () + ';');
             *
             * // Write import for Helper if (!(i instanceof ConstEntry)) stream.println ("import " + i.name () + "Helper;");
             *
             * // Write import for Holder if (!(i instanceof ConstEntry)) if (!(i instanceof TypedefEntry) || (i.type () instanceof
             * SequenceEntry || !((TypedefEntry)i).arrayInfo ().isEmpty ())) stream.println ("import " + i.name () + "Holder;"); }
             */
        }
    } // writePackage
    ...
    static private void addTo(Vector importList, String name) {
        ...
        if (name.startsWith("ValueBase")) // don't import ValueBase*
            if ((name.compareTo("ValueBase") == 0) || (name.compareTo("ValueBaseHolder") == 0) || (name.compareTo("ValueBaseHelper") == 0))
                return;
    ...
    /**
     * Extract the global constants from the supplied integer expression representation (string) and add them to the
     * supplied import list.
     **/
    ...
    public static void writeProlog(PrintWriter stream, String filename) {
        ...
        stream.println("* "
                + Util.getMessage("toJavaProlog2", com.sun.tools.corba.ee.idl.toJavaPortable.Compile.compiler.arguments.file.replace(File.separatorChar, '/')));

        ///////////////
        // This SHOULD work, but there's a bug in the JDK.
        // stream.println ("* " + DateFormat.getDateTimeInstance (DateFormat.FULL, DateFormat.FULL, Locale.getDefault ()).format
        /////////////// (new Date ()));
    ...
    static String parseTerminal(com.sun.tools.corba.ee.idl.constExpr.Terminal e) {
        ...
        /*
         * // Support long long //else if (e.value () instanceof Long) else if (e.value () instanceof BigInteger && (e.type
         * ().indexOf ("long long") >= 0 || e.type ().equals ("unsigned long"))) // <klr> { String rep = e.rep (); int index =
         * rep.indexOf (')'); if (index < 0) return rep + 'L'; else return rep.substring (0, index) + 'L' + rep.substring
         * (index); }
         */
            ...
            if (type.equals("unsigned long long") && ((BigInteger) e.value()).compareTo(com.sun.tools.corba.ee.idl.constExpr.Expression.llMax) > 0) // value >
                                                                                                                                                    // long long
                                                                                                                                                    // Max?
            {
    ...
    static String parseUnary(com.sun.tools.corba.ee.idl.constExpr.UnaryExpr e) {
        if (!(e.value() instanceof Number))
            ...
            String castString = "";
            if (e.operand().value() instanceof Float || e.operand().value() instanceof Double)
                castString = "(double)";
            // Support long long.
            // else
            // castString = "(long)";
    ...
    // <d62023>
    /**
     * @return true if the current setting of corbaLevel is within delta of the range min &lt;= corbaLevel &lt;= max
     **/
