...
public class TypedefGen implements com.sun.tools.corba.ee.idl.TypedefGen, com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator {
    ...
    public int helperType(int index, String indent, com.sun.tools.corba.ee.idl.toJavaPortable.TCOffsets tcoffsets, String name, SymtabEntry entry,
        ...
        if (!inStruct)
            // <54697>
            // stream.println (indent + name + " = org.omg.CORBA.ORB.init ().create_alias_tc (id (), \"" +
            // Util.stripLeadingUnderscores (td.name ()) + "\", " + name + ");");
            stream.println(
                    indent + name + " = org.omg.CORBA.ORB.init ().create_alias_tc (" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(td, true)
                            + ".id (), \"" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.stripLeadingUnderscores(td.name()) + "\", " + name + ");"); // <d61056>
    ...
    public int read(int index, String indent, String name, SymtabEntry entry, PrintWriter stream) {
        ...
        if (!modifier.equals("")) {
            ...
            String arrayDcl = com.sun.tools.corba.ee.idl.toJavaPortable.Util.sansArrayInfo(baseName.substring(startArray)) + "[]"; // Add an extra set because
                                                                                                                                   // the first gets stripped
                                                                                                                                   // off in the loop.
