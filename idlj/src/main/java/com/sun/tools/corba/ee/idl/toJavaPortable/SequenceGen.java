...
public class SequenceGen implements com.sun.tools.corba.ee.idl.SequenceGen, com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator {
        ...
        if (offsetOfType >= 0) {
            ...
            if (maxSize == null)
                ...
                stream.println(indent + name + " = org.omg.CORBA.ORB.init ().create_recursive_sequence_tc ("
                        + com.sun.tools.corba.ee.idl.toJavaPortable.Util.parseExpression(maxSize) + ", " + (offsetOfType - tcoffsets.currentOffset()) + ");");
            ...
            if (maxSize == null)
                ...
                stream.println(indent + name + " = org.omg.CORBA.ORB.init ().create_sequence_tc ("
                        + com.sun.tools.corba.ee.idl.toJavaPortable.Util.parseExpression(maxSize) + ", " + name + ");");
        }
    ...
    public int read(int index, String indent, String name, SymtabEntry entry, PrintWriter stream) {
        ...
        String seqOfName;
        try {
            seqOfName = com.sun.tools.corba.ee.idl.toJavaPortable.Util
                    .sansArrayInfo((String) seq.dynamicVariable(com.sun.tools.corba.ee.idl.toJavaPortable.Compile.typedefInfo));
        ...
        if (seq.type() instanceof PrimitiveEntry)
            ...
            if (seq.type().name().equals("any") || seq.type().name().equals("TypeCode") || seq.type().name().equals("Principal")) {
                ...
                stream.println(indent + "istream.read_" + com.sun.tools.corba.ee.idl.toJavaPortable.Util.collapseName(entry.type().name()) + "_array ("
                        + varName + ", 0, " + length + ");");
            }
    ...
    public int write(int index, String indent, String name, SymtabEntry entry, PrintWriter stream) {
        ...
        if (entry.type() instanceof PrimitiveEntry)
            ...
            if (entry.type().name().equals("any") || entry.type().name().equals("TypeCode") || entry.type().name().equals("Principal")) {
            ...
            String loopIndex = "_i" + index++;
            stream.println(indent + "for (int " + loopIndex + " = 0;" + loopIndex + " < " + name + ".length; ++" + loopIndex + ')');
            stream.println(indent + "  " + com.sun.tools.corba.ee.idl.toJavaPortable.Util.helperName(seq.type(), true) + ".write (ostream, " + name + '['
                    + loopIndex + "]);"); // <d61056>
        }
        return index;
    } // write
