...
public class ValueRepositoryId {
    ...
    /**
     * Add a type to the list of types which have already been included. Note that the type should be added prior to its
     * value.
     *
    ...
    /**
     * Check to see if a specified type has already been processed. If so, add the appropriate 'previously processed' code
     * (0xFFFFFFFF) and sequence offset, and return false; otherwise add the symbol table entry and current offset to the
     * hashtable and return false.
     *
     * @param entry the type to be checked
     * @return true if the symbol table entry has not been previously added; and false otherwise.
     */
    ...
    /**
     * Get the hashcode computed for the value type. This method MUST not be called until all fields have been added, since
     * it computes the hash code from the values entered for each field.
     *
     * @return the 64 bit hashcode for the value type represented as a 16 character hexadecimal string.
     */
    public String getHashcode() {
        if (hashcode == null) {
            byte[] digest = sha.digest();
            hashcode = hexOf(digest[0]) + hexOf(digest[1]) + hexOf(digest[2]) + hexOf(digest[3]) + hexOf(digest[4]) + hexOf(digest[5]) + hexOf(digest[6])
                    + hexOf(digest[7]);
        }
        return hashcode;
    } // getHashCode
