...
public class Helper implements AuxGen {
    ...
    /**
     * Generate the helper class. Provides general algorithm for auxiliary binding generation:
     *
     * 1.) Initialize symbol table and symbol table entry members, common to all generators. 2.) Initialize members unique
     * to this generator. 3.) Open print stream 4.) Write class heading: package, prologue, class statement, open curly 5.)
     * Write class body: member data and methods 6.) Write class closing: close curly 7.) Close the print stream
     **/
    ...
    protected void writeInstVars() {
        stream.println("  private static String  _id = \""
                + com.sun.tools.corba.ee.idl.toJavaPortable.Util.stripLeadingUnderscoresFromID(entry.repositoryID().ID()) + "\";");
        if (entry instanceof ValueEntry) {
            stream.println();
    ...
    protected void writeType() {
        boolean canRecurse = entry instanceof ValueEntry || entry instanceof ValueBoxEntry || entry instanceof StructEntry;
        ...
        if (canRecurse) {
            ...
            ((com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator) entry.generator()).helperType(0, "          ",
                    new com.sun.tools.corba.ee.idl.toJavaPortable.TCOffsets(), "__typeCode", entry, stream);
        } else
            ((com.sun.tools.corba.ee.idl.toJavaPortable.JavaGenerator) entry.generator()).helperType(0, "      ",
                    new com.sun.tools.corba.ee.idl.toJavaPortable.TCOffsets(), "__typeCode", entry, stream);
