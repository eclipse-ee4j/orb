...
public class InterfaceEntry extends com.sun.tools.corba.ee.idl.SymtabEntry implements InterfaceType {
    ...
    /**
     * Add an InterfaceEntry to the list of interfaces which this interface is derivedFrom. During parsing, the parameter to
     * this method COULD be a ForwardEntry, but when parsing is complete, calling derivedFrom will return a vector which
     * only contains InterfaceEntry's.
     */
     ...
     * corresponds to the derivedFrom vector. The first element of this vector is the name of the first element of the
     * derivedFrom vector, etc.
     */
    ...
    /**
     * This is a vector of SymtabEntry's. Valid entries in this vector are: AttributeEntry, ConstEntry, EnumEntry,
     * ExceptionEntry, MethodEntry, StructEntry, NativeEntry, TypedefEntry, UnionEntry. Note that the methods vector is a
     * subset of this vector.
     */
    ...
    void addParentType(com.sun.tools.corba.ee.idl.SymtabEntry e, com.sun.tools.corba.ee.idl.Scanner scanner) {
        if (e instanceof com.sun.tools.corba.ee.idl.ForwardEntry)
            ...
            for (Enumeration enumeration = derivedFrom._allMethods.elements(); enumeration.hasMoreElements();) {
    ...
    /**
     * This method returns a vector of the elements in the state block. If it is null, this is not a stateful interface. If
     * it is non-null, but of zero length, then it is still stateful; it has no state entries itself, but it has an ancestor
     * which does.
     */
    public Vector state() {
        return _state;
