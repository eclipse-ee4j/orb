 ...
 * <LI>string == separated identifiers, such as "localonly", "abstract", or "init". D59407: NOTE: any non-white-space is
 * grouped as part of the identifier.
 * </UL>
 *
 * This pragma handler places a vector of Strings into the dynamicVariable() part of the SymtabEntry. The key to access
 * the dynamicVariable() is MetaPragma.metaKey
 *
 * It is possible to associate a meta pragma with a forward entry. At some point after the parser has completed, the
 * method processForward(ForwardEntry entry) should be called for each ForwardEntry so that the meta information can be
 * folded from the ForwardEntry into the corresponding InterfaceEntry.
 */
...
// System.out.println("exception in MetaPragma");
    ...
    static public void processForward(ForwardEntry forwardEntry) {
        ...
        if (forwardMeta != null && forwardInterface != null) {
            ...
            if (interfaceMeta == null) {
                ...
                /*
                 * The above check is needed because sometimes a forward entry is processed more the once. Not sure why
                 */
                /* merge */
     ...
     * @param msg: string of comma separated message, perhaps with comment. This is implemented as a state machine as
     * follows:
     *
     * State token next action ----------------------------------------------------- initial whitespace initial initial
     * SlashStar comment initial SlashSlash final initial no more final initial text text add to text buffer initial
     * StarSlash initial comment StarSlash initial comment SlashStar comment comment whitespace comment comment SlashSlash
     * comment comment text comment comment no more final text text text add to buffer text SlashStar comment put in vector
     * text whitespace initial put in vector text SlashSlash final put in vector text StarSlash initial put in vector text
     * no more final put in vector
     *
     */
