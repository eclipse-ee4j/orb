...
public class Stub implements AuxGen {
    ...
    /**
     * Steps done within writeBody include: 1.) makeCtors (); 2.) buildMethodList (); 3.) makeMethods (); 4.)
     * makeCORBAObjectMethods ()
     **/
    ...
    protected void writeCtors() {
        ...
        /***
         * the constructors are not generated as per ptc/00-01-08.pdf since these are non-standard APIs, and same can be
         * accomplished programatically, we need to comment this out, in order to be able to generate standard stubs
         */

        /*************
         * stream.println (" // Constructors"); stream.println (" // NOTE: If the default constructor is used, the");
         * stream.println (" // object is useless until _set_delegate (...)"); stream.println (" // is called."); stream.println
         * (" public _" + name + classSuffix + " ()"); stream.println (" {"); stream.println (" super ();"); stream.println ("
         * }"); stream.println (); stream.println (" public _" + name + classSuffix + " (org.omg.CORBA.portable.Delegate
         * delegate)"); stream.println (" {"); stream.println (" super ();"); stream.println (" _set_delegate (delegate);");
         * stream.println (" }");
         ***************/
        ...
        if (localStub) {
            stream.println("  final public static java.lang.Class _opsClass = " + name + "Operations.class;");
            stream.println();
        }
        stream.println();
    } // writeCtors
