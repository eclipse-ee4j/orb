...
class Scanner {
    ...
    // Scanner (IncludeEntry file, String[] keywords, boolean vbose, boolean scanStateful, boolean emitAllIncludes) throws
    // IOException
    // <f60858.1>
     ...
     *
     **/
    Scanner(com.sun.tools.corba.ee.idl.IncludeEntry file, String[] keywords, boolean vbose, boolean emitAllIncludes, float cLevel, boolean debug)
            throws IOException {
        readFile(file);
        verbose = vbose;
        // <f46082.51>
    ...
    private boolean wildcardAtEitherEnd(String string) {
        return string.startsWith("*") || string.startsWith("+") || string.startsWith(".") || string.endsWith("*") || string.endsWith("+")
                || string.endsWith(".");
    } // wildcardAtEitherEnd
    ...
    com.sun.tools.corba.ee.idl.Token getToken() throws IOException {
        ...
        // if (token.type == Token.Identifier || token.type == Token.MacroIdentifier || (token.type >= Token.BooleanLiteral &&
    ...
    /**
     * Starting at a quote, reads a string with possible unicode or octal values until an end quote. Doesn't handle line
     * feeds or comments.
     */
    ...
    private com.sun.tools.corba.ee.idl.Token getString() throws IOException {
        ...
        if (!escaped) { // Escaped id ==> ignore keyword check
            com.sun.tools.corba.ee.idl.Token result = com.sun.tools.corba.ee.idl.Token.makeKeywordToken(string, corbaLevel, escapedOK, collidesWithKeyword);
            if (result != null)
                return result;
        }
        ...
        if (data.ch == '(') {
            readChar();
            return new com.sun.tools.corba.ee.idl.Token(com.sun.tools.corba.ee.idl.Token.MacroIdentifier, string, escaped, collidesWithKeyword[0], false);
        } else
            return new com.sun.tools.corba.ee.idl.Token(com.sun.tools.corba.ee.idl.Token.Identifier, string, escaped, collidesWithKeyword[0], false);
    }
    ...
    private boolean isHexChar(char hex) {
        return ((data.ch >= '0') && (data.ch <= '9')) || ((data.ch >= 'a') && (data.ch <= 'f')) || ((data.ch >= 'A') && (data.ch <= 'F'));
    }
