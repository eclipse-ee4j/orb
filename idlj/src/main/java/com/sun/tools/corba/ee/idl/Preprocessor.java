...
/**
 * This class should be extended if new pragmas are desired. If the preprocessor encounters a pragma name which it
 * doesn't recognize (anything other than ID, prefix, or version), it calls the method otherPragmas. This is the only
 * method which need be overridden. The Preprocessor base class has a number of utility-like methods which can be used
 * by the overridden otherPragmas method.
 **/
public class Preprocessor {
    ...
    private void skipToEndiforElse() throws IOException, com.sun.tools.corba.ee.idl.ParseException {
        while (!token.equals(com.sun.tools.corba.ee.idl.Token.Endif) && !token.equals(com.sun.tools.corba.ee.idl.Token.Else)
                && !token.equals(com.sun.tools.corba.ee.idl.Token.Elif)) {
    ...
    /*
     * These protected methods are used by extenders, by the code which implements otherPragma.
     */
    ...
    /**
     * This method, given an entry name, returns the entry with that name. It can take fully or partially qualified names
     * and returns the appropriate entry defined within the current scope. If no entry exists, null is returned.
     **/
    ...
    /**
     * This method returns a string of all of the characters from the input file from the current position up to, but not
    ...
    /**
     * This method returns a string of all of the characters from the input file from the current position up to, but not
     * including, the given character. It encapsulates parenthesis and quoted strings, meaning it does not stop if the given
     * character is found within parentheses or quotes. For instance, given the input of `start(inside)end', getUntil ('n')
     * will return "start(inside)e"
     **/
    ...
    /**
     * This method assumes that the current token marks the beginning of a scoped name. It then parses the subsequent
     * identifier and double colon tokens, builds the scoped name, and finds the symbol table entry with that name.
     **/
    ...
    /**
     * This method skips the data in the input file until the specified character is encountered, then it returns the next
     * token.
     **/
    ...
    /**
     * This method displays a Parser Exception complete with line number and position information with the given message
     * string.
     **/
    ...
    private String removeDoublePound(String string) {
        ...
        while (index != -1) {
            ...
            if (index != -1) {
                ...
                while (startSkip > 0 && (string.charAt(startSkip) == ' ' || string.charAt(startSkip) == '\t'))
                    --startSkip;
                while (stopSkip < string.length() - 1 && (string.charAt(stopSkip) == ' ' || string.charAt(stopSkip) == '\t'))
                    ++stopSkip;
    ...
    /**
     * This method is called when the parser encounters a left curly brace. An extender of PragmaHandler may find scope
     * information useful. For example, the prefix pragma takes effect as soon as it is encountered and stays in effect
     * until the current scope is closed. If a similar pragma extension is desired, then the openScope and closeScope
     * methods are available for overriding.
     *
     * @param entry the symbol table entry whose scope has just been opened. Be aware that, since the scope has just been
     * entered, this entry is incomplete at this point.
     **/
    ...
    /**
     * This method is called when the parser encounters a right curly brace. An extender of PragmaHandler may find scope
     * information useful. For example, the prefix pragma takes effect as soon as it is encountered and stays in effect
     * until the current scope is closed. If a similar pragma extension is desired, then the openScope and closeScope
     * methods are available for overriding.
     *
