...
/**
 * This factory constructs the default symbol table entries, namely, those declared within the package
...
public class DefaultSymtabFactory implements SymtabFactory {
    ...
    /**
     * "name" can be, but is not limited to, the primitive idl type names: 'char', 'octet', 'short', 'long', etc. The reason
     * it is not limited to these is that, as an extender, you may wish to override these names. For instance, when
     * generating Java code, octet translates to byte, so there is an entry in Compile.overrideNames: &lt;"octet",
     * "byte"&gt; and a PrimitiveEntry in the symbol table for "byte".
     */
