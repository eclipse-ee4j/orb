...
/**
 * Class Token represents a lexeme appearing within an IDL source. Every Token has a type. Depending on its type and on
 * the supported version of IDL, a Token will have other associated attributes, such as a name (identifier, e.g.), and
 * whether it is escaped, deprecated, or is a type that is known to be in a future version of IDL.
 **/
class Token {
    ///////////////
    // Available types
    ...
    Any = 0, // 2.2
            ...
            Case = 3, // .
            Char = 4, // .
            Const = 5, Context = 6, Default = 7, Double = 8, Enum = 9, Exception = 10, FALSE = 11, Fixed = 12, // New addition
            Float = 13, In = 14, Inout = 15, Interface = 16, Long = 17, Module = 18, Native = 19, // New addition
            Object = 20, Octet = 21, Oneway = 22, Out = 23, Raises = 24, Readonly = 25, Sequence = 26, Short = 27, String = 28, Struct = 29, Switch = 30,
            TRUE = 31, Typedef = 32, Unsigned = 33, // .
            Union = 34, // .
            Void = 35, // .
            Wchar = 36, // |
            Wstring = 37, // 2.2
            ...
            Component = 48, Consumes = 49, Emits = 50, Finder = 51, GetRaises = 52, Home = 53, Import = 54, Local = 55, Manages = 56, Multiple = 57,
            PrimaryKey = 58, Provides = 59, Publishes = 60, SetRaises = 61, TypeId = 62, TypePrefix = 63, Uses = 64,

            Identifier = 80, // Identifier
            MacroIdentifier = 81, // Macro Identifier

            Semicolon = 100, // Symbols
            LeftBrace = 101, RightBrace = 102, Colon = 103, Comma = 104, Equal = 105, Plus = 106, Minus = 107, LeftParen = 108, RightParen = 109,
            LessThan = 110, GreaterThan = 111, LeftBracket = 112, RightBracket = 113, Apostrophe = 114, Quote = 115, Backslash = 116, Bar = 117, Carat = 118,
            Ampersand = 119, Star = 120, Slash = 121, Percent = 122, Tilde = 123, DoubleColon = 124, ShiftLeft = 125, ShiftRight = 126, Period = 127,
            Hash = 128, Exclamation = 129, DoubleEqual = 130, NotEqual = 131, GreaterEqual = 132, LessEqual = 133, DoubleBar = 134, DoubleAmpersand = 135,

            BooleanLiteral = 200, // Literals
            CharacterLiteral = 201, IntegerLiteral = 202, FloatingPointLiteral = 203, StringLiteral = 204, Literal = 205,

            Define = 300, // Directives
            Undef = 301, If = 302, Ifdef = 303, Ifndef = 304, Else = 305, Elif = 306, Include = 307, Endif = 308, Line = 309, Error = 310, Pragma = 311,
            Null = 312, Unknown = 313,
    ...
    static final String[] Keywords = { "any", "attribute", "boolean", "case", "char", "const", "context", "default", "double", "enum", "exception", "FALSE",
            "fixed", "float", "in", "inout", "interface", "long", "module", "native", "Object", "octet", "oneway", "out", "raises", "readonly", "sequence",
            "short", "string", "struct", "switch", "TRUE", "typedef", "unsigned", "union", "void", "wchar", "wstring", "init", // In 2.3 only
            "abstract", "custom", "private", // 2.3 and 2.4rtf
            "public", "supports", "truncatable", "ValueBase", "valuetype", "factory", // In 2.4rtf only
            // CORBA 3.0 keywords
            "component", "consumes", "emits", "finder", "getRaises", "home", "import", "local", "manages", "multiple", "primaryKey", "provides", "publishes",
            "setRaises", "supports", "typeId", "typePrefix", "uses" };
    ...
    private static final int FirstKeyword = Any, // 0
            LastKeyword = Uses;

    // <f60858.1> Keywords in CORBA 2.2 that we support.
    private static final int First22Keyword = Any, // 0
            Last22Keyword = Wstring;

    // <f60858.1> New keywords in CORBA 2.3 (preliminary) that we support.
    private static final int First23Keyword = Init, Last23Keyword = Valuetype;
    ...
    // keyword scan.
    private static final int First24rtfKeyword = Abstract, Last24rtfKeyword = Factory;

    // New keywords in CORBA 3.0 (from CORBA components v. 1)
    private static final int First30Keyword = Component, Last30Keyword = Uses;
    ...
    /**
     * Create a keyword token from a string. Determines whether the string is an IDL keyword based on the corbaLevel.
     * Strings that are keywords at higher CORBA levels than the corbaLevel argument create identifier tokens that are
     * marked as "collidesWithKeyword", unless escapedOK is FALSE, which is the case only when preprocessing is taking
     * place. In the case of the "init" keyword, which was only defined in CORBA 2.3, init is marked deprecated in CORBA 2.3
     * since it is not supported in higher levels.
     *
     ...
     * @param boolean escapedOK Flag set true if _ is used to escape an IDL keyword for use as an identifier.
     * @param boolean[] collidesWithKeyword is an array containing one value: a flag representing whether this string is an
     * identifier that collides with a keyword. This is set by this method.
    ...
    static final int FirstSymbol = 100, LastSymbol = 199;
    ...
    static final int FirstLiteral = 200, LastLiteral = 299;

    static final String[] Literals = { Util.getMessage("Token.boolLit"), Util.getMessage("Token.charLit"), Util.getMessage("Token.intLit"),
            Util.getMessage("Token.floatLit"), Util.getMessage("Token.stringLit"), Util.getMessage("Token.literal") };
    ...
    static final int FirstDirective = 300, LastDirective = 399;

    static final String[] Directives = { "define", "undef", "if", "ifdef", "ifndef", "else", "elif", "include", "endif", "line", "error", "pragma", "" };

    // Directives
    ///////////////
    // Specials

    static final int FirstSpecial = 400, LastSpecial = 499;
     ...
     * Constructor.
     *
     * @return a Token having the supplied attribtues. having
     **/
     ...
     *
     * @return true iff the types and names of this and the supplied Token are equal.
     **/
     ...
     * Accessor.
     *
     * @return true iff this token is an identifier having a name matching a keyword in a version of CORBA greater than the
     * specified CORBA level, or iff it matches a keyword in letter, but note case.
     **/
     ...
     * Accessor.
     *
     * @return true iff this token is a deprecated lexeme or lexical type with respect to the specified CORBA level.
     **/
    ...
    /**
     * True iff this token is an identifier that is known to be a keyword in another version of CORBA or matches a keyword
