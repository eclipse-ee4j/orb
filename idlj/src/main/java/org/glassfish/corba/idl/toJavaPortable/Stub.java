...
public class Stub implements AuxGen
{
  ...
  protected void openStream ()
  {
    ...
    String pkg = org.glassfish.corba.idl.toJavaPortable.Util.containerFullName(i.container());
    if (pkg != null && !pkg.equals (""))
    {
      org.glassfish.corba.idl.toJavaPortable.Util.mkdir(pkg);
      name = pkg + '/' + name;
    }
    stream = org.glassfish.corba.idl.toJavaPortable.Util.getStream(name.replace('/', File.separatorChar) + ".java", i);
  } // openStream
  ...
  protected void writeClassDeclaration ()
  {
    ...
    stream.println (" implements " + org.glassfish.corba.idl.toJavaPortable.Util.javaName(i));
  } // writeClassDeclaration
  ...
  protected void writeMethods ()
  {
    ...
    if( (((org.glassfish.corba.idl.toJavaPortable.Arguments) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments).LocalOptimization )
      && !isAbstract ) 
    {
        stream.println( "    final public static java.lang.Class _opsClass =" );
    ...
    for (int i = 0; i < methodList.size (); ++i)
    {
      ...
      if (!localStub) {
      ((org.glassfish.corba.idl.toJavaPortable.MethodGen)method.generator ()).stub (this.i.name(), isAbstract, symbolTable, method, stream, realI);
      } else {
      ((org.glassfish.corba.idl.toJavaPortable.MethodGen)method.generator ()).localstub (symbolTable, method, stream, realI, this.i);
      }
      if (method instanceof AttributeEntry && !((AttributeEntry)method).readOnly ())
        realI += 2;
      else
