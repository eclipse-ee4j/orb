...
public class ValueGen24 extends ValueGen
{
  ...
  protected void writeTruncatable () // <d60929>
  {
    if (!v.isAbstract ()) {
       ...
       stream.print   ("    " + org.glassfish.corba.idl.toJavaPortable.Util.helperName(v, true) + ".id ()");
       ...
       while (child.isSafe ())
       {
        ...
        stream.print("    \"" + org.glassfish.corba.idl.toJavaPortable.Util.stripLeadingUnderscoresFromID(parent.repositoryID().ID()) + "\"");
        child = parent;
      }
      stream.println();
      stream.println("  };");
  ...
  protected void writeHeading ()
  {
    ...
    for (int i = 0; i < v.derivedFrom ().size (); i++) {
        ...
        if ( ((ValueEntry)parent).isAbstract ()) {
            isw.writeClassName( org.glassfish.corba.idl.toJavaPortable.Util.javaName(parent) ) ;
        }
    }
    ...
    while (enumeration.hasMoreElements())  {
        ...
        String cname = org.glassfish.corba.idl.toJavaPortable.Util.javaName(ie) ;
        if (!ie.isAbstract())
            cname += "Operations" ;
  ...
  protected void writeMethods ()
  {
    ...
    while (e.hasMoreElements ())
    {
      ...
      if (contained instanceof AttributeEntry)
      {
        ...
        ((org.glassfish.corba.idl.toJavaPortable.AttributeGen24)element.generator ()).abstractMethod (symbolTable, element, stream);
      }
      else if (contained instanceof MethodEntry)
      {
        MethodEntry element = (MethodEntry)contained;
        ((org.glassfish.corba.idl.toJavaPortable.MethodGen24)element.generator ()).abstractMethod (symbolTable, element, stream);
      }
      else
      {
  ...
  public int read (int index, String indent, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    if (vParents != null && vParents.size() != 0)
    {
      ...
      if ((!parent.isAbstract ()) && (! org.glassfish.corba.idl.toJavaPortable.Util.javaQualifiedName(parent).equals ("java.io.Serializable"))) // <d60929>
  ...
  {
    ((org.glassfish.corba.idl.toJavaPortable.Factories) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.factories ()).valueFactory ().generate (symbolTable, v);
  } // generateValueFactory
  ...
  {
    ((org.glassfish.corba.idl.toJavaPortable.Factories) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.factories ()).defaultFactory ().generate (symbolTable, v);
  } // generateDefaultFactory
}
