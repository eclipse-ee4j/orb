 ...
 **/
public class Util extends org.glassfish.corba.idl.Util
{
  // <d58319>
  /**
  ...
  {
    return org.glassfish.corba.idl.Util.getVersion("com/sun/tools/corba/ee/idl/toJavaPortable/toJavaPortable");
  } // getVersion
  ...
  {
    org.glassfish.corba.idl.toJavaPortable.NameModifier nm = new NameModifierImpl() ;
    return fileName( entry, nm, extension ) ;
  } // fileName

  public static String fileName (SymtabEntry entry, org.glassfish.corba.idl.toJavaPortable.NameModifier modifier, String extension )
  {
  ...
  public static GenFileStream stream (SymtabEntry entry, org.glassfish.corba.idl.toJavaPortable.NameModifier modifier, String extension )
  {
  ...
  {
    // <f46838.4>
    String absPathName = ((org.glassfish.corba.idl.toJavaPortable.Arguments) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments).targetDir + name;
    if (org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments.keepOldFiles && new File (absPathName).exists ())
      return null;
    else
  ...
  public static String javaName (SymtabEntry entry)
  {
    ...
    if (entry instanceof TypedefEntry || entry instanceof SequenceEntry)
      try
      {
        name = sansArrayInfo ((String)entry.dynamicVariable (org.glassfish.corba.idl.toJavaPortable.Compile.typedefInfo));
      }
  ...
  static void fillInfo (SymtabEntry infoEntry)
  {
    ...
    do
    {
      try
      {
        alreadyHave = entry.dynamicVariable (org.glassfish.corba.idl.toJavaPortable.Compile.typedefInfo) != null;
      }
      catch (NoSuchFieldException e)
      {}
      ...
      if (!alreadyHave)
      {
        ...
        {
          org.glassfish.corba.idl.constExpr.Expression maxSize = ((SequenceEntry)entry).maxSize ();
          if (maxSize == null)
            arrayInfo = arrayInfo + "[]";
          else
    ...
    {
      if (alreadyHave)
        infoEntry.dynamicVariable (org.glassfish.corba.idl.toJavaPortable.Compile.typedefInfo, (String)entry.dynamicVariable (
              org.glassfish.corba.idl.toJavaPortable.Compile.typedefInfo) + arrayInfo);
      else
        infoEntry.dynamicVariable (org.glassfish.corba.idl.toJavaPortable.Compile.typedefInfo, javaName (entry) + arrayInfo);
    }
    catch (NoSuchFieldException e)
    {}
  } // fillInfo
  ...
  public static void writePackage (PrintWriter stream, SymtabEntry entry, String name, short type)
  {
    if (name != null && !name.equals (""))
    {
      ...
      // those types.
      if (!org.glassfish.corba.idl.toJavaPortable.Compile.compiler.importTypes.isEmpty ())
      {
        stream.println ();
        Vector v = addImportLines (entry, org.glassfish.corba.idl.toJavaPortable.Compile.compiler.importTypes, type);
  ...
  public static void writePackage (PrintWriter stream, SymtabEntry entry, short type)
  {
    ...
    if (fullName != null && !fullName.equals (""))
    {
      ...
      // in the unnamed module, add an import statement for each of
      // those types.
      if ((type != HolderFile || entry instanceof TypedefEntry) && !org.glassfish.corba.idl.toJavaPortable.Compile.compiler.importTypes.isEmpty ())
      {
        stream.println ();
        Vector v = addImportLines (entry, org.glassfish.corba.idl.toJavaPortable.Compile.compiler.importTypes, type);
        printImports (v, stream);
  ...
  static private Vector<String> addImportLines (SymtabEntry entry, Vector importTypes, short type)
  {
    ...
    else if (entry instanceof TypedefEntry)
    {
      ...
      if (type == HelperFile)
      {
        ...
        try
        {
          String name = (String)t.dynamicVariable (org.glassfish.corba.idl.toJavaPortable.Compile.typedefInfo);
          int index = name.indexOf ('[');
          if (index >= 0)
            name = name.substring (0, index);
  ...
  {
    String targetDir = ((org.glassfish.corba.idl.toJavaPortable.Arguments) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments).targetDir; // F46838.4
  ...
  public static void writeProlog (PrintWriter stream, String filename)
  {
    ...
    String targetDir = ((org.glassfish.corba.idl.toJavaPortable.Arguments) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments).targetDir;
    if (targetDir != null)
    ...
    stream.println ("* " + Util.getMessage ("toJavaProlog2", org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments.file.replace (File.separatorChar, '/')));
   ...
   *
   **/
  public static String parseExpression (org.glassfish.corba.idl.constExpr.Expression e)
  {
    if (e instanceof org.glassfish.corba.idl.constExpr.Terminal)
      return parseTerminal ((org.glassfish.corba.idl.constExpr.Terminal)e);
    else if (e instanceof org.glassfish.corba.idl.constExpr.BinaryExpr)
      return parseBinary ((org.glassfish.corba.idl.constExpr.BinaryExpr)e);
    else if (e instanceof org.glassfish.corba.idl.constExpr.UnaryExpr)
      return parseUnary ((org.glassfish.corba.idl.constExpr.UnaryExpr)e);
    else
   ...
   *
   **/
  static String parseTerminal (org.glassfish.corba.idl.constExpr.Terminal e)
  {
    ...
    else if (e.value () instanceof org.glassfish.corba.idl.constExpr.Expression)
      return '(' + parseExpression ((org.glassfish.corba.idl.constExpr.Expression)e.value ()) + ')';
    ...
    */
    else if (e.value () instanceof BigInteger)
    {
      ...
      if (type.equals("unsigned long long") &&
          ((BigInteger)e.value ()).compareTo (org.glassfish.corba.idl.constExpr.Expression.llMax) > 0) // value > long long Max?
      {
        // Convert to signed value, which will always be negative.
        BigInteger v = (BigInteger)e.value ();
        v = v.subtract (org.glassfish.corba.idl.constExpr.Expression.twoPow64);
   ...
   *
   **/
  static String parseBinary (org.glassfish.corba.idl.constExpr.BinaryExpr e)
  {
    String castString = "";
    if (e.value () instanceof Float || e.value () instanceof Double)
    {
      castString = "(double)";
      if (!(e instanceof org.glassfish.corba.idl.constExpr.Plus || e instanceof org.glassfish.corba.idl.constExpr.Minus ||
            e instanceof org.glassfish.corba.idl.constExpr.Times || e instanceof org.glassfish.corba.idl.constExpr.Divide))
    ...
    if (e.type ().equals ("unsigned long long"))
    {
      ...
      if (value.compareTo (org.glassfish.corba.idl.constExpr.Expression.llMax) > 0) // value > long long max?
        value = value.subtract (org.glassfish.corba.idl.constExpr.Expression.twoPow64); // Convert to Java long (signed)
   ...
   *
   **/
  static String parseUnary (org.glassfish.corba.idl.constExpr.UnaryExpr e)
  {
    ...
    else if ((e.value () instanceof Float || e.value () instanceof Double) && e instanceof org.glassfish.corba.idl.constExpr.Not)
      return "(UNKNOWN_VALUE)"; // This shouldn't happen if the parser checked the expression types correctly.
    else
    {
      String castString = "";
  ...
  {
    float level = org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments.corbaLevel;
    float delta = 0.001f;
