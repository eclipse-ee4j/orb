...
public class Helper implements AuxGen
{
  ...
  protected void init ()
  {
    ...
    if (entry instanceof ValueBoxEntry)
    {
      ...
      if (mType instanceof PrimitiveEntry)
        helperType = org.glassfish.corba.idl.toJavaPortable.Util.javaName(entry);
      else
        helperType = org.glassfish.corba.idl.toJavaPortable.Util.javaName(mType);
    }
    else
      helperType = org.glassfish.corba.idl.toJavaPortable.Util.javaName(entry);
  } // init
  ...
  {
    stream = org.glassfish.corba.idl.toJavaPortable.Util.stream(entry, "Helper.java");
  } // openStream
  ...
  protected void writeType ()
  {
    ...
    ((org.glassfish.corba.idl.toJavaPortable.JavaGenerator)entry.generator ()).helperType (0, "          ", new org.glassfish.corba.idl.toJavaPortable.TCOffsets(), "__typeCode", entry, stream);
    }
    else
    ((org.glassfish.corba.idl.toJavaPortable.JavaGenerator)entry.generator ()).helperType (0, "      ", new org.glassfish.corba.idl.toJavaPortable.TCOffsets(), "__typeCode", entry, stream);
  ...
  protected void writeRead ()
  {
    ...
    if ( !isLocalInterface ) { // nonLocal Interface and other types
      ((org.glassfish.corba.idl.toJavaPortable.JavaGenerator)entry.generator ()).helperRead (helperType, entry, stream);
  ...
  protected void writeWrite ()
  {
    ...
    stream.println ("  {");
    if ( !isLocalInterface ) { // nonLocal Interface and other types
      ((org.glassfish.corba.idl.toJavaPortable.JavaGenerator)entry.generator ()).helperWrite (entry, stream);
    } else { //Local interface should throw exception
      stream.println ("      throw new org.omg.CORBA.MARSHAL ();");
    }
    stream.println ("  }");
  ...
  {
    if ( !org.glassfish.corba.idl.toJavaPortable.Util.IDLEntity(entry))
      return;
  ...
  protected String stubName (InterfaceEntry entry)
  {
    ...
    {
      name = org.glassfish.corba.idl.toJavaPortable.Util.containerFullName(entry.container()) + "._" + entry.name () + "Stub";
    }
    return name.replace ('/', '.');
  } // stubName
