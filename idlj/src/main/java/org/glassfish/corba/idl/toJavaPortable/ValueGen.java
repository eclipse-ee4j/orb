 ...
 *
 **/
public class ValueGen implements org.glassfish.corba.idl.ValueGen, org.glassfish.corba.idl.toJavaPortable.JavaGenerator
{
  /**
  ...
  protected void init ()
  {
    emit = ((Arguments) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments).emit;
    factories = (org.glassfish.corba.idl.toJavaPortable.Factories) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.factories ();
  } // init
  ...
  {
    boolean tie = ((Arguments) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments).TIEServer;
    if (v.supports ().size () > 0  && tie)
    {
      org.glassfish.corba.idl.toJavaPortable.Factories factories = (org.glassfish.corba.idl.toJavaPortable.Factories) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.factories ();
  ...
  protected void writeHeading ()
  {
    ...
    String parentName = org.glassfish.corba.idl.toJavaPortable.Util.javaName(parent);
    boolean impl = false;
    ...
    for (int i = 0; i < v.derivedFrom ().size (); i++) {
      ...
      if ( ((ValueEntry)parent).isAbstract ())
      {
        if (!impl)
        {
          ...
          stream.print (", ");
        stream.print (org.glassfish.corba.idl.toJavaPortable.Util.javaName(parent));
      }
    }
...
// for supported IDL interface
    if (((ValueEntry)v).supports ().size () > 0) {
      ...
      if (s.isAbstract ())
         stream.print (org.glassfish.corba.idl.toJavaPortable.Util.javaName(s));
      else
          stream.print (org.glassfish.corba.idl.toJavaPortable.Util.javaName(s) + "Operations");
      }
...
//  <d59418> Custom valuetypes implement org.omg.CORBA.CustomMarshal.
  ...
  protected void writeTruncatable () // <d60929>
  {
   ...
   if (!v.isAbstract ()) {
        ...
        stream.println ("      return " + org.glassfish.corba.idl.toJavaPortable.Util.helperName(v, true) + ".get_instance().get_truncatable_base_ids();"); // <d61056>
        stream.println ("  }");
        stream.println ();
    }
  } // writeTruncatable
  ...
  protected void writeMembers ()
  {
    ...
    for (int i = 0; i < v.state ().size (); i ++)
    {
      ...
      org.glassfish.corba.idl.toJavaPortable.Util.fillInfo(entry);
      ...
      if (member.modifier == InterfaceState.Public)
        modifier = "  public ";
      org.glassfish.corba.idl.toJavaPortable.Util.writeInitializer(modifier, entry.name(), "", entry, stream);
    }
  } // writeMembers
  ...
  protected void writeInitializers ()
  {
    ...
    if (init != null)
    {
      ...
      for (int i = 0; i < init.size (); i++)
      {
        ...
        ((org.glassfish.corba.idl.toJavaPortable.MethodGen) element.generator ()). interfaceMethod (symbolTable, element, stream);
  ...
  protected void writeMethods ()
  {
    ...
    if (v.supports ().size () > 0)
    {
      ...
      while (el.hasMoreElements ())
      {
        ...
        mClone.container (v);
        ((org.glassfish.corba.idl.toJavaPortable.MethodGen)mClone.generator ()).interfaceMethod (symbolTable, mClone, stream);
      }
    }
  ...
  protected void writeStreamableMethods ()
  {
    ...
    stream.println ("  {");
    stream.println ("    return " + org.glassfish.corba.idl.toJavaPortable.Util.helperName(v, false) + ".type ();"); // <d61056>
    stream.println ("  }");
  } // writeStreamableMethods
  ...
  public int helperType (int index, String indent, org.glassfish.corba.idl.toJavaPortable.TCOffsets tcoffsets, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    for (int k=0; k<noOfMembers; k++)
    {
      ...
      SymtabEntry mType = org.glassfish.corba.idl.toJavaPortable.Util.typeOf(member);
      if (hasRepId (member))
      {
        repId = org.glassfish.corba.idl.toJavaPortable.Util.helperName(mType, true) + ".id ()"; // <d61056>
      ...
      index = ((org.glassfish.corba.idl.toJavaPortable.JavaGenerator)member.generator ()).type (index, indent, tcoffsets, tcOfMembers, member, stream);
  ...
  private static String getConcreteBaseTypeCode (ValueEntry vt)
  {
    ...
    if (!vt.isAbstract ())
    {
      ...
      if (!"ValueBase".equals (base.name ()))
        return org.glassfish.corba.idl.toJavaPortable.Util.helperName(base, true) + ".type ()"; // <d61056>
    }
    return "null";
  ...
  public int read (int index, String indent, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    if (vParents != null && !vParents.isEmpty())
    {
      ...
      if (! org.glassfish.corba.idl.toJavaPortable.Util.javaQualifiedName(parent).equals ("java.io.Serializable")) // <d60929>
          stream.println(indent + org.glassfish.corba.idl.toJavaPortable.Util.helperName(parent, true) + ".read (istream, value);"); // <d60929> // <d61056>
    }
    ...
    for (int k = 0; k < noOfMembers; k++)
    {
      ...
      if (mType instanceof PrimitiveEntry ||
        ...
        index = ((org.glassfish.corba.idl.toJavaPortable.JavaGenerator)member.generator ()).read (index, indent, name + '.' + memberName, member, stream);
      else if (mType instanceof ValueEntry)
      {
        String returnType = org.glassfish.corba.idl.toJavaPortable.Util.javaQualifiedName(mType);
        if (mType instanceof ValueBoxEntry)
          // <f46082.51> Remove -stateful.
          //returnType = Util.javaStatefulName (mType);
          returnType = org.glassfish.corba.idl.toJavaPortable.Util.javaName(mType);
        stream.println ("    " + name + '.' + memberName + " = (" + returnType +
                        ") ((org.omg.CORBA_2_3.portable.InputStream)istream).read_value (" + org.glassfish.corba.idl.toJavaPortable.Util.helperName(mType, true) +  // <d61056>
                        ".get_instance ());"); // <d61056>
      }
      else
        stream.println (indent + name + '.' + memberName + " = " +
                        org.glassfish.corba.idl.toJavaPortable.Util.helperName(mType, true) + ".read (istream);"); // <d61056>
    }
  ...
  public void helperWrite (SymtabEntry entry, PrintWriter stream)
  {
    ...
    if (!((ValueEntry)entry).isCustom ())
    {
       stream.println ("  public static void _write (org.omg.CORBA.portable.OutputStream ostream, " + org.glassfish.corba.idl.toJavaPortable.Util.javaName(entry) + " value)");
       stream.println ("  {");
...
//      write (0, "    ", "value", entry, stream); <d62062>
  ...
  public int write (int index, String indent, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    if (vParents != null && !vParents.isEmpty())
    {
      ...
      if (! org.glassfish.corba.idl.toJavaPortable.Util.javaQualifiedName(parent).equals ("java.io.Serializable")) // <d60929>
          stream.println(indent + org.glassfish.corba.idl.toJavaPortable.Util.helperName(parent, true) + "._write (ostream, value);"); // <d60929> <d61056> <d62062>
    }
    ...
    for (int k = 0; k < noOfMembers; k++)
    {
      ...
      if (mType instanceof PrimitiveEntry ||
          ...
          mType instanceof StringEntry    ||
          !member.arrayInfo ().isEmpty ())
        index = ((org.glassfish.corba.idl.toJavaPortable.JavaGenerator)member.generator ()).write (index, indent, name + '.' + memberName, member, stream);
      else
        stream.println (indent + org.glassfish.corba.idl.toJavaPortable.Util.helperName(mType, true) + // <d61056>
  ...
  protected void writeAbstract ()
  {
    ...
    if (v.supports ().size () > 0)
    {
      ...
      stream.print (org.glassfish.corba.idl.toJavaPortable.Util.javaName(intf));
    }
    stream.println ();
    stream.println ("{");
  }
  ...
  protected org.glassfish.corba.idl.toJavaPortable.Factories factories   = null;
