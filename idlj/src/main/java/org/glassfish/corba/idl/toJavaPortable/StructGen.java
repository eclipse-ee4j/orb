 ...
 *
 **/
public class StructGen implements org.glassfish.corba.idl.StructGen, org.glassfish.corba.idl.toJavaPortable.JavaGenerator
{
  /**
  ...
  {
    stream = org.glassfish.corba.idl.toJavaPortable.Util.stream(s, ".java");
  } // openStream
  ...
  {
    ((Factories) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.factories ()).helper ().generate (symbolTable, s);
  } // generateHelper
  ...
  {
    ((Factories) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.factories ()).holder ().generate (symbolTable, s);
  } // generateHolder
  ...
  {
    org.glassfish.corba.idl.toJavaPortable.Util.writePackage(stream, s);
  ...
  protected void writeMembers ()
  {
    ...
    for (int i = 0; i < s.members ().size (); ++i)
    {
      ...
      org.glassfish.corba.idl.toJavaPortable.Util.fillInfo(member);
      // Transfer member comment to target <31jul1997>.
      if (member.comment () != null)
         member.comment ().generate ("  ", stream);
      org.glassfish.corba.idl.toJavaPortable.Util.writeInitializer("  public ", member.name(), "", member, stream);
    }
  } // writeMembers
    ...
    if (!init || (s.members ().size () > 0))
    {
      ...
      for (int i = 0; i < s.members ().size (); ++i)
      {
        ...
        if (firstTime)
          ...
          stream.print (", ");
        stream.print (org.glassfish.corba.idl.toJavaPortable.Util.javaName(member) + " _" + member.name ());
      }
      stream.println (")");
      stream.println ("  {");
      // fixed mapping for exceptions
  ...
  public int helperType (int index, String indent, org.glassfish.corba.idl.toJavaPortable.TCOffsets tcoffsets, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    for (int i = 0; i < s.members ().size (); ++i)
    {
      ...
      stream.println (indent + "  \"" + org.glassfish.corba.idl.toJavaPortable.Util.stripLeadingUnderscores(memberName) + "\",");
    ...
    stream.println (indent + name + " = org.omg.CORBA.ORB.init ().create_" + (thisIsReallyAnException ? "exception" : "struct") + "_tc (" + org.glassfish.corba.idl.toJavaPortable.Util.helperName(s, true) + ".id (), \"" + org.glassfish.corba.idl.toJavaPortable.Util.stripLeadingUnderscores(entry.name()) + "\", " + membersName + ");"); // <d61056>
    return index;
  } // helperType
  ...
  public int read (int index, String indent, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    while (e.hasMoreElements ())
    {
      ...
      if (!member.arrayInfo ().isEmpty () || mType instanceof SequenceEntry ||
        ...
        index = ((org.glassfish.corba.idl.toJavaPortable.JavaGenerator)member.generator ()).read (index, indent, name + '.' + member.name (), member, stream);
      else if (mType instanceof ValueBoxEntry)
      {
        ...
        {
          jName = org.glassfish.corba.idl.toJavaPortable.Util.javaName(vbType);      // name of mapped Java type
          ...
          jHelper = org.glassfish.corba.idl.toJavaPortable.Util.helperName(mType, true);
              }
        else
        {
          jName = org.glassfish.corba.idl.toJavaPortable.Util.javaName(mType);       // name of mapped Java class
          // <d59437>
          //jHelper = Util.helperName (mType, false);  // <d61056>
          jHelper = org.glassfish.corba.idl.toJavaPortable.Util.helperName(mType, true);
        }
        // <d62023> Call xHelper.read() for valueboxes for RTF2.4
        if (org.glassfish.corba.idl.toJavaPortable.Util.corbaLevel(2.4f, 99.0f))
      ...
      // everything else
          ...
          !org.glassfish.corba.idl.toJavaPortable.Util.corbaLevel(2.4f, 99.0f)) // <d62023>
      {
        // call read_value instead of Helper.read for the value
        stream.println (indent + name + '.' + member.name () + " = (" + org.glassfish.corba.idl.toJavaPortable.Util.javaName(mType) + ") ((org.omg.CORBA_2_3.portable.InputStream)istream).read_value (" + org.glassfish.corba.idl.toJavaPortable.Util.helperName(mType, false) + ".get_instance ());"); // <d60929> // <d61056>
      }
      else
        stream.println (indent + name + '.' + member.name () + " = " + org.glassfish.corba.idl.toJavaPortable.Util.helperName(member.type(), true) + ".read (istream);"); // <d61056>
    }
    return index;
  } // read
  ...
  public int write (int index, String indent, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    for (int i = 0; i < members.size (); ++i)
    {
      ...
      if (!member.arrayInfo ().isEmpty () || mType instanceof SequenceEntry ||
        ...
        index = ((org.glassfish.corba.idl.toJavaPortable.JavaGenerator)member.generator ()).write (index, "    ", name + '.' + member.name (), member, stream);
      ...
      //                everything else
        ...
        stream.println (indent + "((org.omg.CORBA_2_3.portable.OutputStream)ostream).write_value ((java.io.Serializable) " // <d60929>
                        + name + '.' + member.name () + ", "
                        + org.glassfish.corba.idl.toJavaPortable.Util.helperName(member.type(), true) // <d61056>
                        + ".get_instance ());"); // <d61056>
      }
      else
        stream.println (indent + org.glassfish.corba.idl.toJavaPortable.Util.helperName(member.type(), true) + ".write (ostream, " + name + '.' + member.name () + ");"); // <d61056>
    }
    return index;
  } // write
