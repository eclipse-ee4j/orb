...
public class Holder implements AuxGen
{
  ...
  protected void init ()
  {
    ...
    helperClass = org.glassfish.corba.idl.toJavaPortable.Util.helperName(entry, true); // <d61056>
    if (entry instanceof ValueBoxEntry)
    {
      ValueBoxEntry v = (ValueBoxEntry) entry;
      TypedefEntry member = ((InterfaceState) v.state ().elementAt (0)).entry;
      SymtabEntry mType =  member.type ();
      holderType = org.glassfish.corba.idl.toJavaPortable.Util.javaName(mType);
    }
    else
      holderType = org.glassfish.corba.idl.toJavaPortable.Util.javaName(entry);
  } // init
  ...
  {
    stream = org.glassfish.corba.idl.toJavaPortable.Util.stream(entry, "Holder.java");
  } // openStream
  ...
  {
    org.glassfish.corba.idl.toJavaPortable.Util.writePackage (stream, entry, org.glassfish.corba.idl.toJavaPortable.Util.HolderFile);
  ...
  protected void writeBody ()
  {
    if (entry instanceof ValueBoxEntry)
      ...
      org.glassfish.corba.idl.toJavaPortable.Util.writeInitializer("  public ", "value", "", entry, stream);
    stream.println ();
    writeCtors ();
    writeRead ();
    writeWrite ();
