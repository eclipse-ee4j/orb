...
{
  private org.glassfish.corba.idl.toJavaPortable.NameModifier skeletonNameModifier ;
  private org.glassfish.corba.idl.toJavaPortable.NameModifier tieNameModifier ;
  ...
  protected void init ()
  {
    tie = ((org.glassfish.corba.idl.toJavaPortable.Arguments) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments).TIEServer ;
    poa = ((org.glassfish.corba.idl.toJavaPortable.Arguments) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments).POAServer ;

    skeletonNameModifier = 
        ((org.glassfish.corba.idl.toJavaPortable.Arguments) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments).skeletonNameModifier ;
    tieNameModifier = 
        ((org.glassfish.corba.idl.toJavaPortable.Arguments) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments).tieNameModifier ;
    
    tieClassName = tieNameModifier.makeName( i.name() ) ; 
    skeletonClassName = skeletonNameModifier.makeName( i.name() ) ;

    intfName = org.glassfish.corba.idl.toJavaPortable.Util.javaName(i);
    ...
    if (i instanceof ValueEntry)
    {
      ...
      intfName = org.glassfish.corba.idl.toJavaPortable.Util.javaName(intf);
    }
  } // init
  ...
  {
    if (tie)
        stream = org.glassfish.corba.idl.toJavaPortable.Util.stream(i, tieNameModifier, ".java") ;
    else
        stream = org.glassfish.corba.idl.toJavaPortable.Util.stream(i, skeletonNameModifier, ".java") ;
  } // openStream

  protected void writeHeading ()
  {
    org.glassfish.corba.idl.toJavaPortable.Util.writePackage (stream, i, org.glassfish.corba.idl.toJavaPortable.Util.StubFile);
    ...
    if (!isLocalInterface) {
        ...
        if (methodList.size () > 0)
        {
          ...
          for (int i = 0; i < methodList.size (); ++i)
          {
            ...
            ((org.glassfish.corba.idl.toJavaPortable.MethodGen)method.generator ()).dispatchSkeleton (symbolTable, method, stream, realI);
  ...
  protected void writeMethodTable ()
  {
    ...
    while (e.hasMoreElements ())
    {
      ...
      {
        stream.println ("    _methods.put (\"_get_" + org.glassfish.corba.idl.toJavaPortable.Util.stripLeadingUnderscores(method.name()) + "\", " + (++count) + ");");
        if (!((AttributeEntry)method).readOnly ())
          stream.println ("    _methods.put (\"_set_" + org.glassfish.corba.idl.toJavaPortable.Util.stripLeadingUnderscores(method.name()) + "\", " + (++count) + ");");
      }
      else
        stream.println ("    _methods.put (\"" + org.glassfish.corba.idl.toJavaPortable.Util.stripLeadingUnderscores(method.name()) + "\", " + (++count) + ");");
    }
    stream.println ("  }");
    stream.println ();
  } // writeMethodTable
  ...
  protected void writeMethods ()
  {
      ...
      for (int i = 0; i < methodList.size (); ++i)
          {
              MethodEntry method = (MethodEntry)methodList.elementAt (i);
              ((org.glassfish.corba.idl.toJavaPortable.MethodGen)method.generator ()).skeleton
  ...
  private void buildIDList (InterfaceEntry entry, Vector<String> list)
  {
    ...
    {
      String id = org.glassfish.corba.idl.toJavaPortable.Util.stripLeadingUnderscoresFromID(entry.repositoryID().ID());
      if (!list.contains (id))
        list.addElement (id);
