 ...
 *
 **/
public class MethodGen implements org.glassfish.corba.idl.MethodGen
{
  ...
  {
    localOptimization = 
        ((Arguments) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.arguments).LocalOptimization;
    ...
    if (m.exceptions ().size () > 0)
    {
      ...
      while (e.hasMoreElements ())
      {
        if (firstTime)
          ...
          stream.print (", ");
        stream.print (org.glassfish.corba.idl.toJavaPortable.Util.javaName((SymtabEntry) e.nextElement()));
      }
    }
    else
      stream.print (')');
  ...
  protected void writeParmType (SymtabEntry parm, int passType)
  {
    ...
    {
      parm = org.glassfish.corba.idl.toJavaPortable.Util.typeOf(parm);
      stream.print (org.glassfish.corba.idl.toJavaPortable.Util.holderName(parm));
    }
    else // passType is `in'
      // <f46082.51> Remove -stateful feature; javaStatefulName() obsolete.
      //stream.print (Util.javaStatefulName (parm));
      stream.print (org.glassfish.corba.idl.toJavaPortable.Util.javaName(parm));
  } // writeParmType
  ...
  protected void writeDispatchCall ()
  {
    ...
    SymtabEntry mtype = org.glassfish.corba.idl.toJavaPortable.Util.typeOf(m.type());
    ...
    while (parms.hasMoreElements ())
    {
      ...
      if (passType == ParameterEntry.In)
        org.glassfish.corba.idl.toJavaPortable.Util.writeInitializer(indent, name, "", type, writeInputStreamRead("in", type), stream);

      else // the parm is a holder
      {
        String holderName = org.glassfish.corba.idl.toJavaPortable.Util.holderName(type);
    ...
    if (mtype != null)
      org.glassfish.corba.idl.toJavaPortable.Util.writeInitializer(indent, "$result", "", mtype, stream);
    ...
    if (m.exceptions ().size () > 0)
    {
      ...
      while (exceptions.hasMoreElements ())
      {
        ...
        String fullName = org.glassfish.corba.idl.toJavaPortable.Util.javaQualifiedName(exc);
        stream.println (indent + "} catch (" +  fullName + " $ex) {");
        indent = indent + "  ";
        stream.println (indent + "out = $rh.createExceptionReply ();");
        stream.println (indent + org.glassfish.corba.idl.toJavaPortable.Util.helperName(exc, true) + ".write (out, $ex);"); // <d61056>
      }
  ...
  protected void writeStubBody ( String className )
  {
    ...
    String methodName = org.glassfish.corba.idl.toJavaPortable.Util.stripLeadingUnderscores(m.name());
    if (m instanceof AttributeEntry)
    {
      if (m.type () == null)          // if it's a modifier
    ...
    {
      ParameterEntry parm = (ParameterEntry)parms.nextElement ();
      SymtabEntry parmType = org.glassfish.corba.idl.toJavaPortable.Util.typeOf(parm.type());
            ...
            (parm.passType () == ParameterEntry.Inout))
        {
          ...
          if (string.maxSize () != null)
          {
            ...
            stream.println (".length () > (" + 
                org.glassfish.corba.idl.toJavaPortable.Util.parseExpression(string.maxSize()) + "))");
            stream.println (THREE_INDENT + 
    ...
    if (mtype != null)
      org.glassfish.corba.idl.toJavaPortable.Util.writeInitializer(FOUR_INDENT, "$result", "", mtype,
    ...
    if (m.exceptions ().size () > 0)
    {
      ...
      while (exceptions.hasMoreElements ())
      {
        ...
        stream.println (FIVE_INDENT + "throw " + 
            org.glassfish.corba.idl.toJavaPortable.Util.helperName((SymtabEntry) exc, false) + ".read ($in);");
      }
      ...
      String methodName ) 
  {
    ...
    if ( !voidReturnType ) {
        stream.println (FOUR_INDENT + org.glassfish.corba.idl.toJavaPortable.Util.javaName(this.m.type()) +
            " $result;");
    } 
    if( !isValueInitializer() ) {
        if ( voidReturnType ) {
  ...
  private void writeInsert (String indent, String target, String source, SymtabEntry type, PrintWriter stream)
  {
    ...
    if (type instanceof PrimitiveEntry)
    {
      ...
      stream.println (indent + org.glassfish.corba.idl.toJavaPortable.Util.helperName(type, true) + ".insert (" + source + ", " + target + ");"); // <d61056>
  } // writeInsert
  ...
  private void writeType (String indent, String name, SymtabEntry type, PrintWriter stream)
  {
    ...
    else if (type instanceof StringEntry)
    {
      ...
      if (e == null)
        stream.println (indent + name + " (org.omg.CORBA.ORB.init ().create_" + type.name () + "_tc (" + org.glassfish.corba.idl.toJavaPortable.Util.parseExpression(e) + "));");
     else
        stream.println (indent + name + " (org.omg.CORBA.ORB.init ().create_" + type.name () + "_tc (0));");
    }
    else
      stream.println (indent + name + '(' + org.glassfish.corba.idl.toJavaPortable.Util.helperName(type, true) + ".type ());"); // <d61056>
  } // writeType
  ...
  private void writeExtract (String indent, String target, String source, SymtabEntry type, PrintWriter stream)
  {
    if (type instanceof PrimitiveEntry)
    {
      ...
      stream.println (indent + target + " = " + org.glassfish.corba.idl.toJavaPortable.Util.helperName(type, true) + ".extract (" + source + ");"); // <d61056>
  } // writeExtract
  ...
  private String writeExtract (String source, SymtabEntry type)
  {
    ...
    if (type instanceof PrimitiveEntry)
    {
      ...
      extract = org.glassfish.corba.idl.toJavaPortable.Util.helperName(type, true) + ".extract (" + source + ')'; // <d61056>
    return extract;
  } // writeExtract
  ...
  private void writeOutputStreamWrite (String indent, String oStream, String name, SymtabEntry type, PrintWriter stream)
  {
    ...
    else if (type instanceof ValueBoxEntry)
    {
      ...
      if (mType instanceof PrimitiveEntry && name.endsWith (".value"))
        stream.println (org.glassfish.corba.idl.toJavaPortable.Util.helperName(type, true) + ".write (" + oStream + ", "  // <d61056>
        + " new " + org.glassfish.corba.idl.toJavaPortable.Util.javaQualifiedName(type) + " (" + name + "));"); //<d60929>
      else
        stream.println (org.glassfish.corba.idl.toJavaPortable.Util.helperName(type, true) + ".write (" + oStream + ", " + name + ");"); //<d60929> // <d61056>
    }
    else if (type instanceof ValueEntry)
        stream.println (org.glassfish.corba.idl.toJavaPortable.Util.helperName(type, true) + ".write (" + oStream + ", " + name + ");"); //<d60929> // <d61056>
    else
      stream.println (org.glassfish.corba.idl.toJavaPortable.Util.helperName(type, true) + ".write (" + oStream + ", " + name + ");"); // <d61056>
  ...
  private String writeInputStreamRead (String source, SymtabEntry type)
  {
    ...
    if (type instanceof PrimitiveEntry)
    {
      ...
      read = org.glassfish.corba.idl.toJavaPortable.Util.helperName(type, true) + ".read (" + source + ')'; // <d61056>
    return read;
