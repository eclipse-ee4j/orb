 ...
 *
 **/
public class ValueBoxGen implements org.glassfish.corba.idl.ValueBoxGen, org.glassfish.corba.idl.toJavaPortable.JavaGenerator
{
  /**
  ...
  {
    ((Factories) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.factories ()).helper ().generate (symbolTable, v);
  } // generateHelper
  ...
  {
    ((Factories) org.glassfish.corba.idl.toJavaPortable.Compile.compiler.factories ()).holder ().generate (symbolTable, v);
  } // generateHolder
...
//  stream.println ("public class " + v.name () + " implements org.omg.CORBA.portable.ValueBase, org.omg.CORBA.portable.Streamable");
  ...
  protected void writeBody ()
  {
    ...
    stream.println ("  public " +  org.glassfish.corba.idl.toJavaPortable.Util.javaName(entry) + " value;");
    stream.println ("  public " +  v.name () + " (" + org.glassfish.corba.idl.toJavaPortable.Util.javaName(entry) + " initial)");
    stream.println ("  {");
...
//  writeStreamableMethods ();
  ...
  public int helperType (int index, String indent, org.glassfish.corba.idl.toJavaPortable.TCOffsets tcoffsets, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    index = ((org.glassfish.corba.idl.toJavaPortable.JavaGenerator)mType.generator ()).type (index, indent, tcoffsets, name, mType, stream);
  ...
  //                put real marshalling code in read_value.
    ...
    if (mType instanceof PrimitiveEntry ||
      ...
      stream.println (indent + org.glassfish.corba.idl.toJavaPortable.Util.javaName(mType) + " tmp;"); // <d62023>
      ...
      stream.println (indent + org.glassfish.corba.idl.toJavaPortable.Util.javaQualifiedName(mType) + " tmp = (" +
                      org.glassfish.corba.idl.toJavaPortable.Util.javaQualifiedName(mType) + ") ((org.omg.CORBA_2_3.portable.InputStream)istream).read_value (" + org.glassfish.corba.idl.toJavaPortable.Util.helperName(mType, true) + ".get_instance ());"); // <d60929> // <d61056>
    else
  ...
  public int write (int index, String indent, String name, SymtabEntry entry, PrintWriter stream)
  {
    ...
    if (mType instanceof PrimitiveEntry || !member.arrayInfo ().isEmpty ())
      ...
      stream.println (indent
                      ...
                      +  org.glassfish.corba.idl.toJavaPortable.Util.helperName(mType, true)  // <d61056>
                      + ".get_instance ());"); // <d61056>
    else
  ...
  protected void streamableRead (String entryName, SymtabEntry entry, PrintWriter stream)
  {
        ...
        mType instanceof StringEntry || !member.arrayInfo ().isEmpty ())
    {
      ...
      ((org.glassfish.corba.idl.toJavaPortable.JavaGenerator)member.generator ()).read (0, "    ", entryName + ".value", member, stream);
    }
    else if (mType instanceof ValueEntry || mType instanceof ValueBoxEntry)
      stream.println ("    " + entryName + ".value = (" + org.glassfish.corba.idl.toJavaPortable.Util.javaQualifiedName(mType) + ") ((org.omg.CORBA_2_3.portable.InputStream)istream).read_value (" + org.glassfish.corba.idl.toJavaPortable.Util.helperName(mType, true) + ".get_instance ());"); // <d60929> // <d61056>
    else
      stream.println ("    " + entryName + ".value = " + org.glassfish.corba.idl.toJavaPortable.Util.helperName(mType, true) + ".read (istream);"); // <d61056>
  } // streamableRead
