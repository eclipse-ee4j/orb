...
public final class IDLTypesUtil {
    ...
    public boolean isArray(Class c) {
        ...
        if (c.isArray()) {
            ...
            arrayType = (isPrimitive(componentType) || isRemoteInterface(componentType) || isEntity(componentType) || isException(componentType)
    ...
    private void validateConstants(final Class c) throws IDLTypeException {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if ((fieldType != java.lang.String.class) && !isPrimitive(fieldType)) {
                String msg = "Constant field '" + next.getName() + "' in class '" + next.getDeclaringClass().getName() + "' has invalid type' " + next.getType()
                        + "'. Constants" + " in RMI/IIOP interfaces can only have primitive" + " types and java.lang.String types.";
                throw new IDLTypeException(msg);
            }
        }
