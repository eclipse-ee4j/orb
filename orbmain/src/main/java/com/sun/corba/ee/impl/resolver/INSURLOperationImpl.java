 ...
 * eventually become extensible, but for now it simply encapsulates the existing implementation. Once the full
 * extensibility is in place, we want this operation to convert string to INSURL, which has mainly a public resolver
 * method that returns an object reference.
 *
 * @author Hemanth
 * @author Ken
 */
public class INSURLOperationImpl implements Operation {
    ...
    /**
     * This static method takes a Stringified IOR and converts it into IOR object. It is the caller's responsibility to only
     * pass strings that start with "IOR:".
     */
    private org.omg.CORBA.Object getIORFromString(String str) {
        ...
        EncapsInputStream s = EncapsInputStreamFactory.newEncapsInputStream(orb, buf, buf.length, orb.getORBData().getGIOPVersion());
        s.consumeEndian();
    ...
    private org.omg.CORBA.Object getIORUsingCorbaloc(INSURL corbalocObject) {
        ...
        while (iterator.hasNext()) {
            ...
            IIOPAddress addr = IIOPFactories.makeIIOPAddress(element.getHost(), element.getPort());
    ...
    /**
     * This is required for corbaname: resolution. Currently we are not caching RootNamingContext as the reference to
     * rootNamingContext may not be Persistent in all the implementations. _REVISIT_ to clear the rootNamingContext in case
     * of COMM_FAILURE.
     *
    ...
    /**
     * A utility method to clear the RootNamingContext, if there is an exception in resolving CosNaming:Name from the
     * RootNamingContext,
     */
