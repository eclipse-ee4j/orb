...
public class CorbaExtension extends StandardLogger {
    ...
    private static final Class<?>[] SYS_EX_CLASSES = { UNKNOWN.class, BAD_PARAM.class, NO_MEMORY.class, IMP_LIMIT.class, COMM_FAILURE.class, INV_OBJREF.class,
            NO_PERMISSION.class, INTERNAL.class, MARSHAL.class, INITIALIZE.class, NO_IMPLEMENT.class, BAD_TYPECODE.class, BAD_OPERATION.class,
            NO_RESOURCES.class, NO_RESPONSE.class, PERSIST_STORE.class, BAD_INV_ORDER.class, TRANSIENT.class, FREE_MEM.class, INV_IDENT.class, INV_FLAG.class,
            INTF_REPOS.class, BAD_CONTEXT.class, OBJ_ADAPTER.class, DATA_CONVERSION.class, OBJECT_NOT_EXIST.class, TRANSACTION_REQUIRED.class,
            TRANSACTION_ROLLEDBACK.class, INVALID_TRANSACTION.class, INV_POLICY.class, CODESET_INCOMPATIBLE.class, REBIND.class, TIMEOUT.class,
            TRANSACTION_UNAVAILABLE.class, BAD_QOS.class, INVALID_ACTIVITY.class, ACTIVITY_COMPLETED.class, ACTIVITY_REQUIRED.class };

    @SuppressWarnings("unchecked")
    private static final List<Constructor<SystemException>> SYS_EX_CONSTRUCTORS = new ArrayList<Constructor<SystemException>>(SYS_EX_CLASSES.length);

    static {
        Class<?>[] ptypes = { String.class, int.class, CompletionStatus.class };

        for (Class<?> cls : SYS_EX_CLASSES) {
            try {
                @SuppressWarnings("unchecked")
                final Constructor<SystemException> cons = (Constructor<SystemException>) cls.getDeclaredConstructor(ptypes);
                SYS_EX_CONSTRUCTORS.add(cons);
            } catch (Exception ex) {
                throw new RuntimeException("Cound not find constructor for " + cls, ex);
            }
    ...
    public int getMinorCode(Method method) {
        ...
        final int base = orbex.omgException() ? SUNVMCID.value : OMGVMCID.value;
    ...
    public String getLogId(Method method) {
        ...
        final String result = String.format("%03d%1d%04d", exceptionId, omgId, minorCode);
    ...
    public Exception makeException(String msg, Method method) {
        try {
            ...
            SystemException result = cons.newInstance(msg, base + minorCode, csv.getCompletionStatus());
    ...
    public String getLoggerName(Class<?> cls) {
        ...
        if (str.length() == 0) {
            ...
            if (str.endsWith(CLASS_NAME_SUFFIX)) {
                str = str.substring(0, str.length() - CLASS_NAME_SUFFIX.length());
            }
