...
public class StubFactoryCodegenImpl extends StubFactoryDynamicBase {
        ...
        synchronized (classData) {
            ...
            if (stubClass == null) {
                ...
                if (System.getSecurityManager() == null) {
                    stubClass = creator.create(pd, loader, pm.getDebug(), pm.getPrintStream());
                } else {
                    stubClass = AccessController.doPrivileged(new PrivilegedAction<Class<?>>() {
                        public Class<?> run() {
    ...
    public org.omg.CORBA.Object makeStub() {
        ...
        try {
            ...
            stub = AccessController.doPrivileged(new PrivilegedExceptionAction<CodegenStubBase>() {
                public CodegenStubBase run() throws Exception {
                    return CodegenStubBase.class.cast(stubClass.newInstance());
                }
            });
        } catch (Exception exc) {
