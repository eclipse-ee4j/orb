...
public final class IORInfoImpl extends LocalObject implements IORInfo, IORInfoExt {
    ...
    /**
     * Creates a new IORInfo implementation. This info object will establish tagged components with the template for the
     * provided IOR Template.
     */
    ...
    /**
     * An ORB service implementation may determine what server side policy of a particular type is in effect for an IOR
     * being constructed by calling the get_effective_policy operation. When the IOR being constructed is for an object
     * implemented using a POA, all Policy objects passed to the PortableServer::POA::create_POA call that created that POA
     * are accessible via get_effective_policy.
     * <p>
     * If a policy for the given type is not known to the ORB, then this operation will raise INV_POLICY with a standard
     * minor code of 2.
     *
    ...
    /**
     * A portable ORB service implementation calls this method from its implementation of establish_components to add a
     * tagged component to the set which will be included when constructing IORs. The components in this set will be
     ...
     * Any number of components may exist with the same component ID.
     * <p>
     * If the given profile ID does not define a known profile or it is impossible to add components to thgat profile,
     * BAD_PARAM is raised with a minor code of TBD_BP + 3.
     *
     * @param tagged_component The IOP::TaggedComponent to add.
    ...
    private void addIORComponentToProfileInternal(TaggedComponent tagged_component, Iterator iterator) {
        ...
        com.sun.corba.ee.spi.ior.TaggedComponent newTaggedComponent = finder.create(orb, tagged_component);
