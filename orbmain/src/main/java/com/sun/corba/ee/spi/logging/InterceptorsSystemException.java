...
public interface InterceptorsSystemException {
    InterceptorsSystemException self = WrapperGenerator.makeWrapper(InterceptorsSystemException.class, CorbaExtension.self);
    ...
    INTERNAL ignoredExceptionInEstablishComponents(@Chain Exception exc, ObjectAdapter oa);

    @Log(level = LogLevel.FINE, id = 24)
    @Message("Exception in components_established method for ObjectAdapter {0}")
    INTERNAL exceptionInComponentsEstablished(@Chain Exception exc, ObjectAdapter oa);
    ...
    INTERNAL ignoredExceptionInAdapterManagerStateChanged(@Chain Exception exc, int managerId, short newState);
    ...
    INTERNAL ignoredExceptionInAdapterStateChanged(@Chain Exception exc, List<ObjectReferenceTemplate> templates, short newState);
