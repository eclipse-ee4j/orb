...
public final class ORBUtility {
    ...
    public static byte chooseEncodingVersion(ORB orb, IOR ior, GIOPVersion gv) {
        ...
        if (orb.getORBData().isJavaSerializationEnabled() && !(gv.lessThan(GIOPVersion.V1_2))) {
    ...
    private static OMGSystemException omgWrapper = OMGSystemException.self;
    ...
    private static boolean isSystemExceptionTypeCode(TypeCode type, ORB orb) {
        ...
        try {
            return (type.kind().value() == TCKind._tk_except && type.member_count() == 3 && type.member_type(0).equal(systemExceptionMembers[0].type)
                    && type.member_type(1).equal(systemExceptionMembers[1].type) && type.member_type(2).equal(systemExceptionMembers[2].type));
        } catch (BadKind ex) {
            return false;
    ...
    private static ValueHandler vhandler = Util.getInstance().createValueHandler();
    ...
    private static String formatStackTraceElement(StackTraceElement ste) {
        return compressClassName(ste.getClassName()) + "." + ste.getMethodName()
                + (ste.isNativeMethod() ? "(Native Method)"
                        : (ste.getFileName() != null && ste.getLineNumber() >= 0 ? "(" + ste.getFileName() + ":" + ste.getLineNumber() + ")"
    ...
    public static synchronized void dprint(java.lang.Object obj, String msg) {
        System.out.println(compressClassName(obj.getClass().getName()) + "(" + getThreadName(Thread.currentThread()) + "): " + msg);
    }
     ...
     *
     * 1) We can't introduce dependencies on the util version from outside of the io/util packages since it will not exist
