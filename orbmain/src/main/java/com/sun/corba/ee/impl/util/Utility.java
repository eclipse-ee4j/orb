...
public final class Utility {
     ...
     * @exception NoSuchObjectException if obj is an implementation which has not been exported.
     */
     ...
     *
     * This code is rather confused. tieCache maps impls to Ties, but loadTie returns a new Tie instance if the impl is
     * already cached. Also this method is useless with dynamic RMI-IIOP, since we just call new ReflectiveTie to create a
     * Tie. Even with static RMI-IIOP this seems a bit questionable. In the static case, I think we want to avoid possibly
     * expensive ClassLoader searches. But this is really a mapping from stub class to corresponding Tie class. The new
     * stub/tie architecture clearly indicates that this responsibility should lie with the StubFactoryFactory, and this
        ...
        synchronized (tieCache) {
            ...
            if (it == null) {
                ...
                try {
                    ...
                    while (result == null && (objClass = objClass.getSuperclass()) != null && objClass != PortableRemoteObject.class
     ...
     */
    static Class loadClassOfType(String className, String remoteCodebase, ClassLoader loader, Class expectedType, ClassLoader expectedTypeClassLoader)
        ...
        try {
            ...
            try {
                ...
                if (!PackagePrefixChecker.hasOffendingPrefix(wpp)) {
                    loadedClass = Util.getInstance().loadClass(wpp, remoteCodebase, loader);
                } else {
     ...
     * @return the loaded class.
     */
    public static Class loadClassForClass(String className, String remoteCodebase, ClassLoader loader, Class relatedType, ClassLoader relatedTypeClassLoader)
            throws ClassNotFoundException {
        if (relatedType == null) {
     ...
     * @param remoteCodebase the codebase to use. May be null.
     *
     * @param onlyMostDerived if true, will fail if cannot load a stub for the first repID in the tie. If false, will walk
     * all repIDs.
     *
    ...
    public static Class loadStubClass(String repID, String remoteCodebase, Class expectedType) throws ClassNotFoundException {
        ...
        try {
            ...
            return loadClassOfType(PackagePrefixChecker.packagePrefix() + className, remoteCodebase, expectedTypeClassLoader, expectedType,
    ...
    public static String tieName(String className) {
        return PackagePrefixChecker.hasOffendingPrefix(tieNameForCompiler(className)) ? PackagePrefixChecker.packagePrefix() + tieNameForCompiler(className)
