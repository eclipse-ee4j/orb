...
/**
 * CDREncapsCodec is an implementation of Codec, as described in orbos/99-12-02, that supports CDR encapsulation version
 * 1.0, 1.1, and 1.2.
 */
public final class CDREncapsCodec extends org.omg.CORBA.LocalObject implements Codec {
    ...
    /*
     *******************************************************************
     * NOTE: CDREncapsCodec must remain immutable! This is so that we can pre-create CDREncapsCodecs for each version of
     * GIOP in CodecFactoryImpl.
     *******************************************************************/
    ...
    /**
     * Convert the given any into a CDR encapsulated octet sequence. If sendTypeCode is true, the type code is sent with the
     * message, as in a standard encapsulation. If it is false, only the data is sent. Either way, the endian type is sent
    ...
    private byte[] encodeImpl(Any data, boolean sendTypeCode) throws InvalidTypeForEncoding {
        ...
        try {
            ...
            EncapsOutputStream cdrOut = OutputStreamFactory.newEncapsOutputStream((com.sun.corba.ee.spi.orb.ORB) orb, giopVersion);
    ...
    /**
     * Decode the given octet sequence into an any based on a CDR encapsulated octet sequence. If the type code is null, it
    ...
    private Any decodeImpl(byte[] data, TypeCode tc) throws FormatMismatch {
        ...
        try {
            ...
            EncapsInputStream cdrIn = EncapsInputStreamFactory.newEncapsInputStream(orb, data, data.length, giopVersion);
