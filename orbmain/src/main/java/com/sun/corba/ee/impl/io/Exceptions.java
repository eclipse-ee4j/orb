...
public interface Exceptions {
    public Exceptions self = WrapperGenerator.makeWrapper(Exceptions.class, StandardLogger.self);
...
// IIOPInputStream
    ...
    @Log(id = IIS_START + 10)
    ClassCastException couldNotAssignObjectToField(@Chain IllegalArgumentException exc, String className, String fieldName);
...
// ObjectStreamClass
    ...
    void couldNotAccessSerialPersistentFields(@Chain Exception e, String name);
    ...
    void fieldTypeMismatch(String cname, String fldName, Class<?> fldType, String rfldName, Class<?> rfldType);

    @Message("Could not find field {1} in class {0}")
    @Log(level = LogLevel.FINE, id = OSC_START + 3)
    void noSuchField(@Chain NoSuchFieldException e, String className, String fieldName);
