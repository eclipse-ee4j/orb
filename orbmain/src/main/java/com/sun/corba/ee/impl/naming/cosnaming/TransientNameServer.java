...
/**
 * Class TransientNameServer is a standalone application which implements a transient name service. It uses the
 * TransientNameService class for the name service implementation, and the BootstrapServer for implementing
 * bootstrapping, i.e., to get the initial NamingContext.
 * <p>
 * The BootstrapServer uses a Properties object specify the initial service object references supported; such as
 * Properties object is created containing only a "NameService" entry together with the stringified object reference for
 * the initial NamingContext. The BootstrapServer's listening port is set by first checking the supplied arguments to
...
public class TransientNameServer {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            ((com.sun.corba.ee.org.omg.CORBA.ORB) corb).register_initial_reference("NamingService", ns);
