...
/**
 * This class consists of : - Remote interface - Impl of the Remote interface - ServantLocator
 *
 * This is needed for getting the information about all the cluster endpoints when an appclient/standalone client
 * enables FOLB. Without this feature, the client always ends up talking to only the endpoints specified as part of the
 * endpoints property. But in reality, the the endpoints property only contains around 2 endpoints for bootstrapping
 * purposes.
 *
 * In this design, we register the following remote object with CosNaming and then look it up in CosNaming to get hold
 * of the cluster instances This is done only once (during the first call to new InitialContext()).
 *
...
public class InitialGroupInfoService {
    ...
    public static class InitialGISImpl extends PortableRemoteObject implements InitialGIS {
        ...
        public List<ClusterInstanceInfo> getClusterInstanceInfo() throws RemoteException {
            ...
            try {
                GroupInfoService gis = (GroupInfoService) PortableRemoteObject
                        .narrow(orb.resolve_initial_references(ORBConstants.FOLB_SERVER_GROUP_INFO_SERVICE), GroupInfoService.class);
    ...
    public void bindName(ORB orb) {
        try {
            ...
            Policy[] arr = new Policy[] { rootPOA.create_servant_retention_policy(ServantRetentionPolicyValue.NON_RETAIN),
            ...
            org.omg.CORBA.Object provider = poa.create_reference_with_id(id, servantLocator.getType());

            // put object in NameService
            org.omg.CORBA.Object objRef = orb.resolve_initial_references("NameService");
            NamingContext ncRef = NamingContextHelper.narrow(objRef);
            NameComponent nc = new NameComponent(ORBConstants.INITIAL_GROUP_INFO_SERVICE, "");
