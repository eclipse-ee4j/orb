...
public class ContactInfoListImpl implements ContactInfoList {
    ...
    public synchronized Iterator<ContactInfo> iterator() {
        ...
        Iterator<ContactInfo> result = new ContactInfoListIteratorImpl(orb, this, primaryContactInfo, rotate(effectiveTargetIORContactInfoList),
                usePerRequestLoadBalancing);

        /*
         * This doesn't work due to some strange behavior in FOLB: we are getting far too many IOR updates. Updates are received
         * even when the cluster shape has not changed.
         */
        /*
         * if (usePerRequestLoadBalancing) { // Copy the list, otherwise we will get a ConcurrentModificationException as //
         * soon as next() is called on the iterator. List<CorbaContactInfo> newList = new ArrayList(
         * effectiveTargetIORContactInfoList ) ; CorbaContactInfo head = newList.remove(0) ; newList.add( head ) ;
    ...
    public synchronized void setEffectiveTargetIOR(IOR newIOR) {
        ...
        Iterator<TaggedComponent> lbcomps = temp.iteratorById(ORBConstants.TAG_LOAD_BALANCING_ID);
        if (lbcomps.hasNext()) {
            LoadBalancingComponent lbcomp = null;
            lbcomp = (LoadBalancingComponent) (lbcomps.next());
            usePerRequestLoadBalancing = lbcomp.getLoadBalancingValue() == ORBConstants.PER_REQUEST_LOAD_BALANCING;
        }
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (this.targetIOR != other.targetIOR && (this.targetIOR == null || !this.targetIOR.equals(other.targetIOR))) {
            return false;
        }
        return true;
    }
    ...
    private void createContactInfoList() {
        ...
        if (effectiveTargetIORContactInfoList == null) {
            effectiveTargetIORContactInfoList = new ArrayList<ContactInfo>();
            ...
            if (isLocal) {
                ...
                ContactInfo contactInfo = new SharedCDRContactInfoImpl(orb, this, effectiveTargetIOR, orb.getORBData().getGIOPAddressDisposition());
            ...
            if (!isLocal) {
                ...
                display("Subsequent time for (colocated) iiopProfile", iiopProfile);
            }
        }
    ...
    private void addRemoteContactInfos(IOR effectiveTargetIOR, List<ContactInfo> effectiveTargetIORContactInfoList) {
        ...
        List<? extends SocketInfo> socketInfos = orb.getORBData().getIORToSocketInfo().getSocketInfo(effectiveTargetIOR,
                // 6152681
    ...
    protected ContactInfo createContactInfo(String type, String hostname, int port) {
        ...
        return new ContactInfoImpl(orb, this,
     ...
     *
     * Note: this must be maintained accurately whether or not the ORB allows local optimization, because ServantManagers in
     * the POA ALWAYS use local optimization ONLY (they do not have a remote case).
     */
    @IsLocal
