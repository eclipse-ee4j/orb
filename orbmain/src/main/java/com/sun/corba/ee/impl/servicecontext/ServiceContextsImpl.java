...
public class ServiceContextsImpl implements ServiceContexts {
     ...
     *
     * Keys are java.lang.Integers for service context IDs. Values are either instances of ServiceContext or the unmarshaled
     * byte arrays (unmarshaled on first use).
     *
     * This provides a mild optimization if we don't happen to use a given service context, but it's main advantage is that
     * it allows us to change the order in which we unmarshal them. We need to do the UnknownExceptionInfo service context
     * after the SendingContextRunTime service context so that we can get the CodeBase if necessary.
     */
    ...
    private ServiceContext unmarshal(int scId, byte[] data) {
        ...
        if (factory == null) {
            ...
            EncapsInputStream eis = EncapsInputStreamFactory.newEncapsInputStream(orb, data, data.length, giopVersion, codeBase);
    ...
    /**
     * Write the given entry from the scMap to the OutputStream. See note on giopVersion. The service context should know
     * the GIOP version it is meant for.
     */
    @TraceServiceContext
