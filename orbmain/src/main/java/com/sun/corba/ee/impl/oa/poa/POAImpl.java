...
public class POAImpl extends ObjectAdapterBase implements POA {
    ...
    public String toString() {
        return "POA[" + poaId.toString() + ", uniquePOAId=" + uniquePOAId + ", state=" + stateToString() + ", invocationCount=" + invocationCount.get() + "]";
    }
    ...
    @InfoMethod
    private void initializingPoa(int scid, int serverid, String orbid, ObjectAdapterId poaId) {
    }
    ...
    public POA create_POA(String name, POAManager theManager, Policy[] policies) throws AdapterAlreadyExists, InvalidPolicy {
        ...
        try {
            ...
            try {
                ...
                if (newManager == null) {
                    newManager = new POAManagerImpl(manager.getFactory(), getORB().getPIHandler());
    ...
    private void doActivate(AdapterActivator act, String name, POAImpl child) throws AdapterNonExistent {
        ...
        try {
            ...
            throw omgWrapper.adapterActivatorException(exc, poaName, poaId);
        } catch (Throwable thr) {
    ...
    private void cleanUpLocks(POAImpl child, boolean readLocked, boolean writeLocked, boolean childReadLocked) {
        ...
        if (readLocked || writeLocked || childReadLocked) {
            locksWereHeld();
            wrapper.findPOALocksNotReleased(readLocked, writeLocked, childReadLocked);
    ...
    public void exit() {
        try {
            ...
            lock();
            try {
                if ((invocationCount.decrementAndGet() == 0) && (state == STATE_DESTROYING)) {
                    invokeCV.signalAll();
                }
            } finally {
                unlock();
