...
/**
 * This class represents an Java class declaration. It refers to either a binary or source definition.
 *
 * ClassDefinitions are loaded on demand, this means that class declarations are late bound. The definition of the class
 * is obtained in stages. The status field describes the state of the class definition:
 *
 * CS_UNDEFINED - the definition is not yet loaded CS_UNDECIDED - a binary definition is loaded, but it is still unclear
 * if the source definition need to be loaded CS_BINARY - the binary class is loaded CS_PARSED - the class is loaded
 * from the source file, the type information is available, but the class has not yet been compiled. CS_CHECKED - the
 * class is loaded from the source file and has been type-checked. CS_COMPILED - the class has been type checked,
 * compiled, and written out. CS_NOTFOUND - no class definition could be found
 *
...
public final class ClassDeclaration implements Constants {
    ...
    /**
     * This is a flag for use by getClassDefinition(env). It is used to mark that a class has been successfully looked up by
     * that method before.
     */
    private boolean found = false;

    /**
     * Get the definition of this class, if the class is not yet defined, load the definition. Loading a class may throw
     * various exceptions.
     */
    public ClassDefinition getClassDefinition(Environment env) throws ClassNotFound {
        if (tracing)
            env.dtEvent("getClassDefinition: " + getName() + ", status " + getStatus());
    ...
    /**
     * Get the definition of this class, if the class is not yet defined, load the definition. Loading a class may throw
     * various exceptions. Perform no basicCheck() on this class.
     */
