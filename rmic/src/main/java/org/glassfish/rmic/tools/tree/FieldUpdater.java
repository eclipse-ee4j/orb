...
/**
 * This class encapsulates the information required to generate an update to a private field referenced from another
 * class, e.g., an inner class. An expression denoting a reference to the object to which the field belongs is
 * associated with getter and setter methods.
 * <p>
 * We use this class only for assignment, increment, and decrement operators, in which the old value is first retrieved
 * and then a new value is computed and stored. Simple assignment expressions in which a value is copied without
 * modification are handled by another mechanism.
 *
...
class FieldUpdater implements Constants {
    ...
    /**
     * Since the object reference expression may be captured before it has been inlined, we must inline it later. A
     * <code>FieldUpdater</code> is inlined essentially as if it were a child of the assignment node to which it belongs.
     */
    ...
    /**
     * Duplicate <code>items</code> words from the top of the stack, locating them below the topmost <code>depth</code>
     * words on the stack.
     */
    ...
    /**
     * Begin a field update by an assignment, increment, or decrement operator. The current value of the field is left at
     * the top of the stack. If <code>valNeeded</code> is true, we arrange for the initial value to remain on the stack
     * after the update.
     */
    ...
    /**
     * Complete a field update by an assignment, increment, or decrement operator. The original value of the field left on
     * the stack by <code>startUpdate</code> must have been replaced with the updated value, with no other stack
     * alterations. If <code>valNeeded</code> is true, we arrange for the updated value to remain on the stack after the
     * update. The <code>valNeeded</code> argument must not be true in both <code>startUpdate</code> and
     * <code>finishUpdate</code>.
     */
    ...
    /**
     * Like above, but used when assigning a new value independent of the old, as in a simple assignment expression. After
     * 'startAssign', code must be emitted to leave one additional value on the stack without altering any others, followed
     * by 'finishAssign'.
     */
