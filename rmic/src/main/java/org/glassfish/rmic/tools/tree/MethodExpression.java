...
public class MethodExpression extends NaryExpression {
    ...
    public Vset checkValue(Environment env, Context ctx, Vset vset, Hashtable<Object, Object> exp) {
        ...
        if (isArray && (field.getName() == idClone) && (field.getType().getArgumentTypes().length == 0)) {
            /*
             * Arrays pretend that they have "public Object clone()" that doesn't throw anything, according to the language spec.
             */
    ...
    /**
     * We're about to report a "unmatched method" error. Try to issue a better diagnostic by comparing the actual argument
     * types with the method (or methods) available. In particular, if there is an argument which fails to match
     * <em>any</em> method, we report a type mismatch error against that particular argument. The diagnostic will report a
     * target type taken from one of the methods.
     * <p>
    ...
    public Expression inline(Environment env, Context ctx) {
        ...
        try {
            ...
            // itself might throw NullPointerException as a side effect
                    ...
                    ctxClass.permitInlinedAccess(env, field.getClassDeclaration()) && ctxClass.permitInlinedAccess(env, field)
    ...
    /*
     * Grab all instance initializer code from the class definition, and return as one bolus. Note that we are assuming the
     * the relevant fields have already been checked. (See the pre-pass in SourceClass.checkMembers which ensures this.)
     */
    ...
    public void codeValue(Environment env, Context ctx, Assembler asm) {
        ...
        if (field.isStatic()) {
            ...
            right.codeValue(env, ctx, asm);
            /*****
             * if (right.op == FIELD && ((FieldExpression)right).id == idSuper) { // '<class>.super.<method>(...)' isSuper = true; }
             *****/
        }
