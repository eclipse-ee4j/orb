...
public class FieldExpression extends UnaryExpression {
    ...
    /**
     * Convert a qualified name into a type. Performs a careful check of each inner-class component, including the JLS 6.6.1
     * access checks that were omitted in 'FieldExpression.toType'.
     * <p>
     * This code is similar to 'checkCommon', which could be cleaned up a bit long the lines we have done here.
     */
    /*-------------------------------------------------------*
    ...
    *-------------------------------------------------------*/
    ...
    /**
     * If 'checkAmbiguousName' returns 'Package.tPackage', then it was unable to resolve any prefix of the qualified name.
     * This method attempts to diagnose the problem.
     */
    ...
    private Vset checkCommon(Environment env, Context ctx, Vset vset, Hashtable<Object, Object> exp, UnaryExpression loc, boolean isLHS) {
        ...
        try {
            ...
            if (field.isProtected() && !(right instanceof SuperExpression
                    ...
                    || (right instanceof FieldExpression && ((FieldExpression) right).id == idSuper)) && !sourceClass.protectedAccess(env, field, right.type)) {
    ...
    /**
     * Return a <code>FieldUpdater</code> object to be used in updating the value of the location denoted by
     * <code>this</code>, which must be an expression suitable for the left-hand side of an assignment. This is used for
     * implementing assignments to private fields for which an access method is required. Returns null if no access method
     * is needed, in which case the assignment is handled in the usual way, by direct access. Only simple assignment
     * expressions are handled here Assignment operators and pre/post increment/decrement operators are are handled by
     * 'getUpdater' below.
     * <p>
     ...
     * Return a <code>FieldUpdater</code> object to be used in updating the value of the location denoted by
     * <code>this</code>, which must be an expression suitable for the left-hand side of an assignment. This is used for
     * implementing the assignment operators and the increment and decrement operators on private fields that are accessed
     * from another class, e.g, uplevel from an inner class. Returns null if no access method is needed.
     * <p>
     * Must be called after 'checkValue', else 'right' will be invalid.
     */
    ...
    private Vset checkInnerClass(Environment env, Context ctx, Vset vset, Hashtable<Object, Object> exp, UnaryExpression loc) {
        ...
        try {
            ...
            if (field.isProtected() && !(right instanceof SuperExpression
                    // Extension of JLS 6.6.2 for qualified 'super'.
                    || (right instanceof FieldExpression && ((FieldExpression) right).id == idSuper)) && !ctxClass.protectedAccess(env, field, right.type)) {
                env.error(where, "invalid.protected.field.use", field.getName(), field.getClassDeclaration(), right.type);
                return vset;
            }
    ...
    /**
     * There is a simple assignment being made to the given final field. The field was named either by a simple name or by
     * an almost-simple expression of the form "this.v". Check if this is a legal assignment.
     * <p>
     * Blank final variables can be set in initializers or constructor bodies. In all cases there must be definite single
     * assignment. (All instance and instance variable initializers and each constructor body are treated as if concatenated
     * for the purposes of this check. Assignment to "this.x" is treated as a definite assignment to the simple name "x"
     * which names the instance variable.)
     */
