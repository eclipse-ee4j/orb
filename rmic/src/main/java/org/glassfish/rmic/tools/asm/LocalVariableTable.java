...
final class LocalVariableTable {
    ...
    /**
     * Trim overlapping local ranges. Java forbids shadowing of locals in nested scopes, but non-nested scopes may still
     * declare locals with the same name. Because local variable ranges are computed using flow analysis as part of
     * assembly, it isn't possible to simply make sure variable ranges end where the enclosing lexical scope ends. This
     * method makes sure that variables with the same name don't overlap, giving priority to fields with higher slot numbers
     * that should have appeared later in the source.
     */
    ...
    void write(Environment env, DataOutputStream out, ConstantPool tab) throws IOException {
        ...
        out.writeShort(len);
        for (int i = 0; i < len; i++) {
            // System.out.println("pc=" + locals[i].from + ", len=" + (locals[i].to - locals[i].from) + ", nm=" +
            // locals[i].field.getName() + ", slot=" + locals[i].slot);
