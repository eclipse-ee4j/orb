...
/**
 * This interface defines constant that are used throughout the compiler. It inherits from RuntimeConstants, which is an
 * autogenerated class that contains contstants defined in the interpreter.
 *
...
public interface Constants extends RuntimeConstants {
    ...
    /*
     * Enable/disable inclusion of certain debug tracing code in the compiler. When included, the tracing code may be
     * selectively enabled at runtime, otherwise we save the space/time overhead. Should normally be 'false' for a release
     * version.
     */
...
// end JCOV
    ...
    /*
     * This is not a real signature marker, since it is also an identifier constituent character.
     */
...
// end JCOV
     ...
     * Modifiers.
     *
     * There has been much confusion regarding modifiers. There are a number of distinct usages:
     *
     * - in classfiles to annotate classes, as per JVM pg. 102. - in classfiles to annotate methods, as per JVM pg. 104. -
     * in classfiles to annotate InnerClass attributes, as per http://java.sun.com/products/jdk/1.1/docs/guide/innerclasses
     * - in the compiler to record java source level modifiers, as per JLS pg. 157 et al., plus misc. info such as whether a
     * method is deprecated - in the JVM to record misc. info, such as whether a method has has been compiled
     *
     * To make matters worse, the terms "access flags" and "modifiers" are often used interchangably, and some information
     * that might make sense as a flag is expressed using attributes (ie. Synthetic).
     *
     * The constants defined herein have been divided by whether they make sense only within the compiler (M_* and MM_*) or
     * whether they only make sense to the JVM (ACC_* and ACCM_*). At an earlier time these were all lumped together. Future
     * maintenance should strive to keep the distinction clear.
     *
     * Note that modifier M_STRICTFP is not in general recoverable from the ACC_STRICT bit in classfiles.
     *
     * Note also that the modifiers M_LOCAL and M_ANONYMOUS do not appear in the InnerClass attribute, as they are above the
     * first 16 bits.
     */
...
// end JCOV
    ...
    /*
     * Number of bits used in file offsets. The line number and file offset are concatenated into a long, with enough room
     * for other information to be added later if desired (such as token lengths). For the moment explicit bit manipulations
     * are used to modify the fields. This makes sense for efficiency but at some point these ought to be better
     * encapsulated.
     */
     ...
     * Operator precedence
     */
    int opPrecedence[] = { 10, 11, 11, 11, 11, 11, 11, 11, 11, 11, 11, 11, 11, 12, 13, 14, 15, 16, 17, 18, 18, 19, 19, 19, 19, 19, 20, 20, 20, 21, 21, 22, 22,
            22, 23, 24, 24, 24, 24, 24, 24, 25, 25, 26, 26, 26, 26, 26, 26 };

    /*
     * Operator names
     */
    String opNames[] = { ",", "=", "*=", "/=", "%=", "+=", "-=", "<<=", ">>=", ">>>=", "&=", "|=", "^=", "?:", "||", "&&", "|", "^", "&", "!=", "==", ">=", ">",
            "<=", "<", "instanceof", "<<", ">>", ">>>", "+", "-", "/", "%", "*", "cast", "+", "-", "!", "~", "++", "--", "new", "new", "new", "++", "--",
            ...
            "if", "else", "for", "while", "do", "switch", "case", "default", "break", "continue", "return", "try", "catch", "finally", "throw", "stat",
            "expression", "declaration", "declaration", null,

            "import", "class", "extends", "implements", "interface", "package", null, null, null, null,

            "private", "public", "protected", "const", "static", "transient", "synchronized", "native", "final", "volatile", "abstract", "strictfp", null, null,
            null,

            ";", ":", "?", "{", "}", "(", ")", "[", "]", "throws", "error", "comment", "type", "length", "inline-return", "inline-method", "inline-new" };
}
