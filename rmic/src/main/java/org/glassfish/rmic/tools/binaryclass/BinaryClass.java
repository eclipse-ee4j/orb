...
public final class BinaryClass extends ClassDefinition implements Constants {
     ...
     * Constructor
     */
    private BinaryClass(Object source, ClassDeclaration declaration, int modifiers, ClassDeclaration superClass, ClassDeclaration interfaces[],
            Vector<ClassDeclaration> dependencies) {
    ...
    /**
     * Ready a BinaryClass for further checking. Note that, until recently, BinaryClass relied on the default basicCheck()
     * provided by ClassDefinition. The definition here has been added to ensure that the information generated by
     * collectInheritedMethods is available for BinaryClasses.
     */
    ...
    public static BinaryClass load(Environment env, DataInputStream in, int mask) throws IOException {
        ...
        if (version < JAVA_MIN_SUPPORTED_VERSION) {
            throw new ClassFormatError(org.glassfish.rmic.tools.javac.Main.getText("javac.err.version.too.old", String.valueOf(version)));
            ...
            throw new ClassFormatError(org.glassfish.rmic.tools.javac.Main.getText("javac.err.version.too.recent", version + "." + minor_version));
        }
    ...
    /**
     * Called when an environment ties a binary definition to a declaration. At this point, auxiliary definitions may be
     * loaded.
     */
    ...
    private void initInner(ClassDefinition outerClass, int mods) {
        if (getOuterClass() != null)
            return; // already done
        /******
         * // Maybe set static, protected, or private. if ((modifiers & M_PUBLIC) != 0) mods &= M_STATIC; else mods &= M_PRIVATE
         * | M_PROTECTED | M_STATIC; modifiers |= mods;
         ******/
