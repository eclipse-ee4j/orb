...
public abstract class ClassDefinition implements Constants {
     ...
     * Constructor
     */
    protected ClassDefinition(Object source, long where, ClassDeclaration declaration, int modifiers, IdentifierToken superClass,
     ...
     *
     * It is only possible to resolve names at this point if we are a source class. The provision of this method at this
     * level in the class hierarchy is dubious, but see 'getInnerClass' below. All other calls to 'getSuperClass(env)'
     * appear in 'SourceClass'. NOTE: An older definition of this method has been moved to 'SourceClass', where it overrides
     * this one.
     *
    ...
    /**
     * Tell if the class is inner. This predicate also returns true for top-level nested types. To test for a true inner
     * class as seen by the programmer, use {@code !isTopLevel()}.
     */
    ...
    /**
     * Return true if the given documentation string contains a deprecation paragraph. This is true if the string contains
     * the tag @deprecated is the first word in a line.
     */
    ...
    /**
     * Check to see if a class which implements interface `this' could possibly implement the interface `intDef'. Note that
     * the only way that this can fail is if `this' and `intDef' have methods which are of the same signature and different
     * return types. This method is used by Environment.explicitCast() to determine if a cast between two interfaces is
     * legal.
     *
     * This method should only be called on a class after it has been basicCheck()'ed.
     */
    ...
    /**
     * We know the field is marked protected (and not public) and that the field is visible (as per canAccess). Can we
     * access the field as <accessor>.<field>, where <accessor> has the type <accessorType>?
     *
     * Protected fields can only be accessed when the accessorType is a subclass of the current class
     */
    public boolean protectedAccess(Environment env, MemberDefinition f, Type accessorType) throws ClassNotFound {
        ...
        f.isStatic() || // allow array.clone()
                (accessorType.isType(TC_ARRAY) && (f.getName() == idClone) && (f.getType().getArgumentTypes().length == 0)) || // <accessorType> is a subtype of
                                                                                                                               // the current class
                (accessorType.isType(TC_CLASS) && env.getClassDefinition(accessorType.getClassName()).subClassOf(env, getClassDeclaration())) || // we are
                                                                                                                                                 // accessing
                                                                                                                                                 // the field
                                                                                                                                                 // from a
                                                                                                                                                 // friendly
    ...
    /**
     * Get a field from this class. Report ambiguous fields. If no accessible field is found, this method may return an
     * inaccessible field to allow a useful error message.
     *
     * getVariable now takes the source class `source' as an argument. This allows getVariable to check whether a field is
     * inaccessible before it signals that a field is ambiguous. The compiler used to signal an ambiguity even when one of
     * the fields involved was not accessible. (bug 4053724)
     */
    ...
    /*
     * private fields are never inherited. package-private fields are not inherited across package boundaries. To capture
     * this, we take two booleans as parameters: showPrivate indicates whether we have passed a class boundary, and
     * showPackage indicates whether we have crossed a package boundary.
     */
    private MemberDefinition getVariable0(Environment env, Identifier nm, ClassDefinition source, boolean showPrivate, boolean showPackage)
    ...
    /**
     * Note that this class is being used somehow by {@code ref}. Report deprecation errors, etc.
     */
    ...
    /**
     * Get an inner class. Look in supers but not outers. (This is used directly to resolve expressions like "site.K", and
    ...
    /**
     * Lookup a method. This code implements the method lookup mechanism specified in JLS 15.11.2.
     *
     * This mechanism cannot be used to lookup synthetic methods.
     */
    private MemberDefinition matchMethod(Environment env, ClassDefinition accessor, Identifier methodName, Type[] argumentTypes, boolean isAnonConstCall,
            Identifier accessPackage) throws AmbiguousMember, ClassNotFound {
    ...
    /**
     * A version of matchMethod to be used only for constructors when we cannot pass in a sourceClass argument. We just
     * assert our package name.
     *
     * This is used only for anonymous classes, where we have to look up a (potentially) protected constructor with no valid
     * sourceClass parameter available.
     */
     ...
     *
     * Only called by javadoc. For now I am holding off rewriting this code to rely on collectInheritedMethods(), as that
     * code has not gotten along with javadoc in the past.
     */
    ...
    /**
     * This is a workaround to allow javadoc to turn off certain inheritance/override checks which interfere with javadoc
     * badly. In the future it might be good to eliminate the shared sources of javadoc and javac to avoid the need for this
     * sort of workaround.
     */
    ...
    /**
     * Add all of the methods declared in or above `parent' to `allMethods', the set of methods in the current class.
     * `myMethods' is the set of all methods declared in this class, and `mirandaMethods' is a repository for Miranda
     * methods. If mirandaMethods is null, no mirandaMethods will be generated.
     *
     * For a definition of Miranda methods, see the comment above the method addMirandaMethods() which occurs later in this
     * file.
     */
        ...
        try {
            ...
            while (methods.hasNext()) {
                ...
                if (method.isPackagePrivate() && !inSamePackage(method.getClassDeclaration())) {
                    ...
                    if (override != null && this instanceof org.glassfish.rmic.tools.javac.SourceClass) {
                ...
                if (override != null) {
                    ...
                    if (formerMethod == null) {
                        ...
                        if (mirandaMethods != null && pClass.isInterface() && !isInterface()) {
                            ...
                            // VM.
                            method = new org.glassfish.rmic.tools.javac.SourceMember(method, this, env);
    ...
    /**
     * <p>
     * Collect all methods defined in this class or inherited from any of our superclasses or interfaces. Look for any
     * incompatible definitions.
     *
     * <p>
     * This function is also responsible for collecting the <em>Miranda</em> methods for a class. For a definition of
     * Miranda methods, see the comment in addMirandaMethods() below.
     */
    ...
    /**
     * Get an Iterator of all methods which could be accessed in an instance of this class. Throw a compiler error if we
     * haven't generated this information yet.
     */
    ...
    /**
     * Add a list of methods to this class as miranda methods. This gets overridden with a meaningful implementation in
     * SourceClass. BinaryClass should not need to do anything -- it should already have its miranda methods and, if it
     * doesn't, then that doesn't affect our compilation.
     */
    ...
    /**
     * We create a stub for this. Source classes do more work. Some calls from 'SourceClass.checkSupers' execute this
     * method.
     *
     ...
     * <p>
     * This routine operates on a "best-efforts" basis. If at some point a class is not found, the partially-resolved
     * identifier is returned. Eventually, someone else has to try to get the ClassDefinition and diagnose the
     * ClassNotFound.
     * <p>
     * resolveName() looks at surrounding scopes, and hence pulling in both inherited and uplevel types. By contrast,
     * resolveInnerClass() is intended only for interpreting explicitly qualified names, and so look only at inherited
     * types. Also, resolveName() looks for package prefixes, which appear similar to "very uplevel" outer classes.
     * <p>
     * A similar (but more complex) name-lookup process happens when field and identifier expressions denoting qualified
     * names are type-checked. The added complexity comes from the fact that variables may occur in such names, and take
     * precedence over class and package names.
     * <p>
     * In the expression type-checker, resolveInnerClass() is paralleled by code in FieldExpression.checkAmbigName(), which
     * also calls ClassDefinition.getInnerClass() to interpret names of the form "OuterClass.Inner" (and also
     * outerObject.Inner). The checking of an identifier expression that fails to be a variable is referred directly to
     * resolveName().
     */
    ...
    /**
     * Interpret a qualified class name, which may have further subcomponents.. Follow inheritance links, as in: class C {
     * class N { } } class D extends C { } ... new D.N() ... Ignore outer scopes and packages.
     */
    ...
    /**
     * While resolving import directives, the question has arisen: does a given inner class exist? If the top-level class
     * exists, we ask it about an inner class via this method. This method looks only at the literal name of the class, and
     * does not attempt to follow inheritance links. This is necessary, since at the time imports are being processed,
     * inheritance links have not been resolved yet. (Thus, an import directive must always spell a class name exactly.)
     */
    ...
    /**
     * Given the fact that this class has no method "nm" matching "argTypes", find out if the mismatch can be blamed on a
     * particular actual argument which disagrees with all of the overloadings. If so, return the code
     * (i<<2)+(castOK<<1)+ambig, where "i" is the number of the offending argument, and "castOK" is 1 if a cast could fix
     * the problem. The target type for the argument is returned in margTypeResult[0]. If not all methods agree on this
     * type, "ambig" is 1. If there is more than one method, the choice of target type is arbitrary.
     * <p>
     * Return -1 if every argument is acceptable to at least one method. Return -2 if there are no methods of the required
     * arity. The value "start" gives the index of the first argument to begin checking.
     */
    ...
    /**
     * Return the same value as getReferences. Also, mark the set of references frozen. After that, it is an error to add
     * new references.
     */
    ...
    /**
     * Maintain a hash table of local and anonymous classes whose internal names are prefixed by the current class. The key
     * is the simple internal name, less the prefix.
     */
