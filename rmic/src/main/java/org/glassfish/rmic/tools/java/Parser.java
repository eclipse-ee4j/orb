 ...
 * <p>
 *
 * This class implements an operator precedence parser. Errors are reported to the Environment object, if the error
 * can't be resolved immediately, a SyntaxError exception is thrown.
 * <p>
 *
 * Error recovery is implemented by catching SyntaxError exceptions and discarding input tokens until an input token is
 * reached that is possibly a legal continuation.
 * <p>
 *
 * The parse tree that is constructed represents the input exactly (no rewrites to simpler forms). This is important if
 * the resulting tree is to be used for code formatting in a programming environment. Currently only documentation
 * comments are retained.
 * <p>
 *
 * The parsing algorithm does NOT use any type information. Changes in the type system do not affect the structure of
 * the parse tree. This restriction does introduce an ambiguity an expression of the form: (e1) e2 is assumed to be a
 * cast if e2 does not start with an operator. That means that (a) - b is interpreted as subtract b from a and not cast
 * negative b to type a. However, if a is a simple type (byte, int, ...) then it is assumed to be a cast.
 * <p>
 *
...
public class Parser extends Scanner implements ParserActions, Constants {
    ...
    /**
     * Usually <code>this.actions == (ParserActions)this</code>. However, a delegate scanner can produce tokens for this
     * parser, in which case <code>(Scanner)this</code> is unused, except for <code>this.token</code> and
     * <code>this.pos</code> instance variables which are filled from the real scanner by <code>this.scan()</code> and the
     * constructor.
     */
    ...
    /**
     * Report the current class under construction. By default, it's a no-op which returns null. It may only be called
     * before the corresponding endClass().
     */
     ...
     */
    @Deprecated
    public void defineField(long where, ClassDefinition c, String doc, int mod, Type t, IdentifierToken nm, IdentifierToken args[], IdentifierToken exp[],
            Node val) {
        // By default, call the deprecated version.
    ...
    /*
     * A growable array of nodes. It is used as a growable buffer to hold argument lists and expression lists. I'm not using
     * Vector to make it more efficient.
     */
    ...
    /**
     * Recover after a syntax error in a statement. This involves discarding tokens until EOF or a possible continuation is
     * encountered.
     */
    ...
    /**
     * Parse an identifier. ie: a.b.c returns "a.b.c" If star is true then "a.b.*" is allowed. The return value encodes both
     * the identifier and its location.
     */
    ...
    /**
     * Parse the tail of a type expression, which might be array brackets. Return the given type, as possibly modified by
     * the suffix.
     */
    ...
    /*
     * Dealing with argument lists, I'm not using Vector for efficiency.
     */
    ...
    /**
     * Parse a possibly-empty sequence of modifier keywords. Return the resulting bitmask. Diagnose repeated modifiers, but
     * make no other checks. Only modifiers mentioned in the given bitmask are scanned; an unmatched modifier must be
    ...
    /**
     * Recover after a syntax error in a field. This involves discarding tokens until an EOF or a possible legal
     ...
     */
    protected ClassDefinition parseClassBody(IdentifierToken nm, int mod, int ctx, String doc, Vector<IdentifierToken> ext, Vector<IdentifierToken> impl,
            long p) throws SyntaxError, IOException {
        // Decide which is the super class
        IdentifierToken sup = null;
        if ((mod & M_INTERFACE) != 0) {
            if (impl.size() > 0) {
                env.error(impl.elementAt(0).getWhere(), "intf.impl.intf");
            }
            impl = ext;
        } else {
            if (ext.size() > 0) {
                if (ext.size() > 1) {
                    env.error(ext.elementAt(1).getWhere(), "multiple.inherit");
                }
                sup = ext.elementAt(0);
            }
        }
    ...
    /**
     * Recover after a syntax error in the file. This involves discarding tokens until an EOF or a possible legal
     * continuation is encountered.
     */
    ...
    /**
     * Usually <code>this.scanner == (Scanner)this</code>. However, a delegate scanner can produce tokens for this parser,
     * in which case <code>(Scanner)this</code> is unused, except for <code>this.token</code> and <code>this.pos</code>
     * instance variables which are filled from the real scanner by <code>this.scan()</code> and the constructor.
     */
