...
/**
 * This class defines the environment for a compilation. It is used to load classes, resolve class names and report
 * errors. It is an abstract class, a subclass must define implementations for some of the functions.
 * <p>
 *
 * An environment has a source object associated with it. This is the thing against which errors are reported, it is
 * usually a file name, a field or a class.
 * <p>
 *
...
public class Environment implements Constants {
    ...
    /**
     * The object that is currently being parsed/compiled. It is either a file name (String) or a field (MemberDefinition)
     ...
     * <p>
     * Should be called only with 'internal' class names, i.e., the result of a call to 'resolveName' or a synthetic class
     * name.
     */
    ...
    /**
     * Check if a class exists (without actually loading it). (Since inner classes cannot in general be examined without
    ...
    /**
     * Resolve a type. Make sure that all the classes referred to by the type have a definition. Report errors. Return true
     * if the type is well-formed. Presently used for types appearing in member declarations, which represent named types
     * internally as qualified identifiers. Type names appearing in local variable declarations and within expressions are
     * represented as identifier or field expressions, and are resolved by 'toType', which delegates handling of the
     * non-inner portion of the name to this method.
     * <p>
     * In 'toType', the various stages of qualification are represented by separate AST nodes. Here, we are given a single
     * identifier which contains the entire qualification structure. It is not possible in general to set the error location
     * to the exact position of a component that is in error, so an error message must refer to the entire qualified name.
     * An attempt to keep track of the string length of the components of the name and to offset the location accordingly
     * fails because the initial prefix of the name may have been rewritten by an earlier call to 'resolveName'. See
     * 'SourceMember.resolveTypeStructure'. The situation is actually even worse than this, because only a single location
     * is passed in for an entire declaration, which may contain many type names. All error messages are thus poorly
     * localized. These checks should be done while traversing the parse tree for the type, not the type descriptor.
     * <p>
     * DESIGN NOTE: As far as I can tell, the two-stage resolution of names represented in string form is an artifact of the
     * late implementation of inner classes and the use of mangled names internally within the compiler. All qualified names
     * should have their hiearchical structure made explicit in the parse tree at the phase at which they are presented for
     * static semantic checking. This would affect class names appearing in 'extends', 'implements', and 'throws' clauses,
    ...
    /**
     * Given its fully-qualified name, verify that a class is defined and accessible. Used to check components of qualified
     * names in contexts where a class is expected. Like 'resolve', but is given a single type name, not a type descriptor.
     */
    ...
    /**
     * Like 'getClassDefinition(env)', but check access on each component. Currently called only by 'resolve' above. It is
     * doubtful that calls to 'getClassDefinition(env)' are appropriate now.
     */
    ...
    /**
     * Resolve the names within a type, returning the adjusted type. Adjust class names to reflect scoping. Do not report
     * errors.
     * <p>
     * NOTE: It would be convenient to check for errors here, such as verifying that each component of a qualified name
     * exists and is accessible. Why must this be done in a separate phase?
     * <p>
     * If the 'synth' argument is true, indicating that the member whose type is being resolved is synthetic, names are
     * resolved with respect to the package scope. (Fix for 4097882)
     */
    ...
    /**
     * Discover if name consists of a package prefix, followed by the name of a class (that actually exists), followed
     * possibly by some inner class names. If we can't find a class that exists, return the name unchanged.
     * <p>
     * This routine is used after a class name fails to be resolved by means of imports or inner classes. However, import
     * processing uses this routine directly, since import names must be exactly qualified to start with.
     */
     ...
     * Create a new class.
     */
    public ClassDefinition makeClassDefinition(Environment origEnv, long where, IdentifierToken name, String doc, int modifiers, IdentifierToken superClass,
            IdentifierToken interfaces[], ClassDefinition outerClass) {
        if (env == null)
            return null; // lame default
     ...
     * Create a new field.
     */
    public MemberDefinition makeMemberDefinition(Environment origEnv, long where, ClassDefinition clazz, String doc, int modifiers, Type type, Identifier name,
            IdentifierToken argNames[], IdentifierToken expIds[], Object value) {
        if (env == null)
            return null; // lame default
    ...
    /**
     * Debugging flags. There used to be a method debug() that has been replaced because -g has changed meaning (it now
     * cooperates with -O and line number, variable range and source file info can be toggled separately).
     */
    ...
    /**
     * Optimization flags. There used to be a method optimize() that has been replaced because -O has changed meaning in
    ...
    /**
     * Do not support virtual machines before version 1.2. This option is not supported and is only here for testing
     * purposes.
     */
    ...
    /**
     * Issue an error. source - the input source, usually a file name string offset - the offset in the source of the error
     * err - the error number (as defined in this interface) arg1 - an optional argument to the error (null if not
     * applicable) arg2 - a second optional argument to the error (null if not applicable) arg3 - a third optional argument
     * to the error (null if not applicable)
     */
    ...
    /**
     * Output a string. This can either be an error message or something for debugging. This should be used instead of
     * println.
     */
...
// end JCOV
    ...
    /**
     * Debug tracing. Currently, this code is used only for tracing the loading and checking of classes, particularly the
     * demand-driven aspects. This code should probably be integrated with 'debugOutput' above, but we need to give more
     * thought to the issue of classifying debugging messages and allowing those only those of interest to be enabled.
     *
     * Calls to these methods are generally conditioned on the final variable 'Constants.tracing', which allows the calls to
     * be completely omitted in a production release to avoid space and time overhead.
     */
    ...
    /**
     * Enable diagnostic dump of class modifier bits, including those in InnerClasses attributes, as they are written to the
     * classfile. In the future, may also enable dumping field and method modifiers.
     */
