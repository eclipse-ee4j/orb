...
/**
 * A RemoteClass object encapsulates RMI-specific information about a remote implementation class, i.e. a class that
...
public class RemoteClass implements org.glassfish.rmic.RMIConstants {
     ...
     *
     * Returns true if successful. If the class is not a properly formed remote implementation class or if some other error
     * occurs, the return value will be null, and errors will have been reported to the supplied BatchEnvironment.
     */
     ...
     *
     * A remote interface is any interface that extends Remote, directly or indirectly. The remote interfaces of a class are
     * the interfaces directly listed in either the class's "implements" clause, or the "implements" clause of any of its
     * superclasses, that are remote interfaces.
     *
     * The order of the array returned is arbitrary, and some elements may be superfluous (i.e., superinterfaces of other
     ...
     * all of the methods in the class's remote interfaces.
     *
     * The methods in the array are ordered according to the comparison of the strings consisting of their method name
     * followed by their type signature, so each method's index in the array corresponds to its "operation number" in the
    ...
    /**
     * Return the "interface hash" used to match a stub/skeleton pair for this class in the JDK 1.1 version of the
    ...
    /**
     * Validate that the remote implementation class is properly formed and fill in the data structures required by the
     * public interface.
     */
    private boolean initialize() {
        ...
        /*
         * Initialize cached definitions for the Remote interface and the RemoteException class.
         */
        try {
        ...
        /*
         * Here we find all of the remote interfaces of our remote implementation class. For each class up the superclass chain,
         * add each directly-implemented interface that somehow extends Remote to a list.
         */
        ...
        for (ClassDefinition classDef = implClassDef; classDef != null;) {
            try {
                ...
                for (int i = 0; i < interfaces.length; i++) {
                    ...
                    /*
                     * Add interface to the list if it extends Remote and it is not already there.
                     */
                ...
                /*
                 * Verify that the candidate remote implementation class implements at least one remote interface directly.
                 */
                if (classDef == implClassDef && remotesImplemented.isEmpty()) {
                    if (defRemote.implementedBy(env, implClassDef.getClassDeclaration())) {
                        /*
                         * This error message is used if the class does implement a remote interface through one of its superclasses, but not
                         * directly.
                         */
                        env.error(0, "rmic.must.implement.remote.directly", implClassDef.getName());
                    } else {
                        /*
                         * This error message is used if the class never implements a remote interface.
                         */
        ...
        /*
         * The "remotesImplemented" vector now contains all of the remote interfaces directly implemented by the remote class or
         * by any of its superclasses.
         *
         * At this point, we could optimize the list by removing superfluous entries, i.e. any interfaces that are implemented
         * by some other interface in the list anyway.
         *
         * This should be correct; would it be worthwhile?
         *
         * for (int i = 0; i < remotesImplemented.size();) { ClassDefinition interfaceDef = (ClassDefinition)
         * remotesImplemented.elementAt(i); boolean isOtherwiseImplemented = false; for (int j = 0; j < remotesImplemented.size;
         * j++) { if (j != i && interfaceDef.implementedBy(env, (ClassDefinition) remotesImplemented.elementAt(j).
         * getClassDeclaration())) { isOtherwiseImplemented = true; break; } } if (isOtherwiseImplemented) {
         * remotesImplemented.removeElementAt(i); } else { ++i; } }
         */

        /*
         * Now we collect the methods from all of the remote interfaces into a hashtable.
         */
        ...
        /*
         * Convert vector of remote interfaces to an array (order is not important for this array).
         */
        remoteInterfaces = new ClassDefinition[remotesImplemented.size()];
        remotesImplemented.copyInto(remoteInterfaces);

        /*
         * Sort table of remote methods into an array. The elements are sorted in ascending order of the string of the method's
         * name and type signature, so that each elements index is equal to its operation number of the JDK 1.1 version of the
         * stub/skeleton protocol.
         */
        ...
        /**
         * Finally, pre-compute the interface hash to be used by stubs/skeletons for this remote class.
         */
    ...
    /**
     * Collect and validate all methods from given interface and all of its superinterfaces as remote methods. Remote
     * methods are added to the supplied hashtable. Returns true if successful, or false if an error occurred.
     */
    private boolean collectRemoteMethods(ClassDefinition interfaceDef, Hashtable<String, Method> table) {
        ...
        /*
         * rmic used to enforce that a remote interface could not extend a non-remote interface, i.e. an interface that did not
         * itself extend from Remote. The current version of rmic does not have this restriction, so the following code is now
         * commented out.
         *
         * Verify that this interface extends Remote, since all interfaces extended by a remote interface must implement Remote.
         *
         * try { if (!defRemote.implementedBy(env, interfaceDef.getClassDeclaration())) { env.error(0,
         * "rmic.can.mix.remote.nonremote", interfaceDef.getName()); return false; } } catch (ClassNotFound e) { env.error(0,
         * "class.not.found", e.name, interfaceDef.getName()); return false; }
         */
        ...
        nextMember: for (MemberDefinition member = interfaceDef.getFirstMember(); member != null; member = member.getNextMember()) {
            if (member.isMethod() && !member.isConstructor() && !member.isInitializer()) {
                ...
                for (int i = 0; i < exceptions.length; i++) {
                    /*
                     * rmic used to enforce that a remote method had to explicitly list RemoteException in its "throws" clause; i.e., just
                     * throwing Exception was not acceptable. The current version of rmic does not have this restriction, so the following
                     * code is now commented out. Instead, the method is considered valid if RemoteException is a subclass of any of the
                     * methods declared exceptions.
                     *
                     * if (exceptions[i].getName().equals( idRemoteException)) { hasRemoteException = true; break; }
                     */
                    try {
                ...
                /*
                 * If this method did not throw RemoteException as required, generate the error but continue, so that multiple such
                ...
                /*
                 * Verify that the implementation of this method throws only java.lang.Exception or its subclasses (fix bugid 4092486).
                 * JRMP does not support remote methods throwing java.lang.Throwable or other subclasses.
                 */
                try {
                ...
                /*
                 * Create RemoteClass.Method object to represent this method found in a remote interface.
                 */
                Method newMethod = new Method(member);
                /*
                 * Store remote method's representation in the table of remote methods found, keyed by its name and parameter signature.
                 *
                 * If the table already contains an entry with the same method name and parameter signature, then we must replace the
                 * old entry with a Method object that represents a legal combination of the old and the new methods; specifically, the
                 * combined method must have a throws list that contains (only) all of the checked exceptions that can be thrown by both
                 * the old or the new method (see bugid 4070653).
                 */
    ...
    /**
     * Compute the "interface hash" of the stub/skeleton pair for this remote implementation class. This is the 64-bit value
     * used to enforce compatibility between a stub and a skeleton using the JDK 1.1 version of the stub/skeleton protocol.
     *
     * It is calculated using the first 64 bits of a SHA digest. The digest is from a stream consisting of the following
     * data: (int) stub version number, always 1 for each remote method, in order of operation number: (UTF) method name
     * (UTF) method type signature for each declared exception, in alphabetical name order: (UTF) name of exception class
     *
     */
    ...
    /**
     * Sort array of class declarations alphabetically by their mangled fully-qualified class name. This is used to feed a
     * method's exceptions in a canonical order into the digest stream for the interface hash computation.
     */
    ...
    /**
     * A RemoteClass.Method object encapsulates RMI-specific information about a particular remote method in the remote
     * implementation class represented by the outer instance.
     */
    public class Method implements Cloneable {
         ...
         *
         * For methods with the same name and type signature inherited from multiple remote interfaces, the array will contain
         * the set of exceptions declared in all of the interfaces' methods that can be legally thrown in each of them.
         */
        ...
        /**
         * Return the string representation of this method appropriate for the construction of a java.rmi.server.Operation
         * object.
         */
        ...
        /**
         * Return a string consisting of this method's name followed by its method descriptor, using the Java VM's notation for
         * method descriptors (see section 4.3.3 of The Java Virtual Machine Specification).
         */
         ...
         *
         * Note that this member definition may be only one of several member defintions that correspond to this remote method
         * object, if several of this class's remote interfaces contain methods with the same name and type signature.
         * Therefore, this member definition may declare more exceptions thrown that this remote method does.
         */
         ...
         *
         * This list can include superfluous entries, such as unchecked exceptions and subclasses of other entries.
         */
         ...
         */
        /*
         * Temporarily comment out the private modifier until the VM allows outer class to access inner class's private
         * constructor
         */
         ...
         *
         * This requires determining the exceptions declared by the combined method, which must be (only) all of the exceptions
         * declared in both old Methods that may thrown in either of them.
         */
        ...
        /**
         * Compute the "method hash" of this remote method. The method hash is a long containing the first 64 bits of the SHA
         * digest from the UTF encoded string of the method name and descriptor.
         *
         * REMIND: Should this method share implementation code with the outer class's computeInterfaceHash() method?
         */
