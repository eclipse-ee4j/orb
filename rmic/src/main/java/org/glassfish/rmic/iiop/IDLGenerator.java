...
public class IDLGenerator extends org.glassfish.rmic.iiop.Generator {
     ...
     */
    protected org.glassfish.rmic.iiop.CompoundType getTopType(ClassDefinition cdef, ContextStack stack) {
        return CompoundType.forCompound(cdef, stack);
    }

    /**
     * Return an Identifier which contains the fully-qualified IDL filename for the given OutputType. The OutputType
     * contains a filename string (not directory) and Type.
     *
    ...
    protected Identifier getOutputId(OutputType ot) {
        ...
        if ("org.omg.CORBA.Object".equals(t.getQualifiedName()) && t.isArray()) // array of org.omg.CORBA.Object
    ...
    public boolean parseArgs(String argv[], Main main) {
        ...
        String idlTo;
        if (result) {
            nextArg: for (int i = 0; i < argv.length; i++) {
                if (argv[i] != null) {
    ...
    /**
     * Return an array of OutputTypes for the IDL files that need to be generated for the given top-level type. OutputTypes
     * contain filename string (not directory) and Type.
     *
    ...
    protected Vector getAllReferencesFor(CompoundType ct) {
        ...
        nextSequence: while (e.hasMoreElements()) {
    ...
    /**
     * Accumulate and filter all those types that are referenced by the given referenced types. Keep only those for which
     * IDL is to be generated.
     *
    ...
    /**
     * Determine if IDL should be generated for a referenced type. Do not generate IDL for a CORBA Object reference. It gets
     * mapped to the original IDL or to Object (if exactly org.omg.CORBA.Object) Generate (boxed) IDL for an IDL Entity
     * unless it is an IDL user exception, a ValueBase, an AbstractBase (or a CORBA Object). Do not generate IDL for
     * Implementation classes..unless they inherit from multiple distinct remote interfaces
     *
    ...
    protected boolean isIDLGeneratedFor(CompoundType t) {
        ...
        if (t.isIDLEntity())
            if (t.isBoxed())
                return true;
            else if ("org.omg.CORBA.portable.IDLEntity".equals(t.getQualifiedName()))
                return true;
    ...
    /**
     * Write the output for the given OutputFileName into the output stream. (The IDL mapping for java.lang.Class is
     * generated from javax.rmi.CORBA.ClassDesc in the tools workspace)
     *
     ...
     */
    protected void writeOutputFor(OutputType ot, HashSet alreadyChecked, IndentingWriter p) throws IOException {
        Type t = ot.getType();
    ...
    /**
     * Write an IDL valuetype definition for 1) a nonconforming Java class 2) a nonconforming Java interface (that is not an
     * AbstractType)
     *
    ...
    /**
     * Write an IDL interface definition for either: 1) a conforming Java remote interface (RemoteType)..or 2) a
     * non-conforming Java interface whose methods all throw java.rmi.RemoteException (AbstractType)
     *
    ...
    /**
     * Write an IDL valuetype definition for a conforming Java class. Methods and constructors are optional..controlled by
     * -valueMethods flag
     *
     ...
     * @param p The output stream.
     */
    protected void writeProlog(CompoundType t, Hashtable refHash, Hashtable spcHash, Hashtable arrHash, Hashtable excHash, Hashtable inhHash, IndentingWriter p)
            throws IOException {
        writeBanner(t, 0, !isException, p);
    ...
    protected void getInterfaces(CompoundType ct, Hashtable inhHash) {
        ...
        nextInterface: for (int i1 = 0; i1 < infs.length; i1++) { // forall inherited interfaces
            String inhName = infs[i1].getQualifiedName();
            switch (ct.getTypeCode()) {
            case TYPE_NC_CLASS:
            case TYPE_VALUE: // filter for classes
                if ("java.io.Externalizable".equals(inhName) || "java.io.Serializable".equals(inhName) || "org.omg.CORBA.portable.IDLEntity".equals(inhName))
    ...
    /**
     * Add reference for given type avoiding duplication. Sort into specials, arrays and regular references. Filter out
     * types which are not required.
     *
    ...
    /**
     * Determine whether given Type is a special reference. Special cases are: java.io.Serializable, java.io.Externalizable,
     * java.lang.Object, java.rmi.Remote and org.omg.CORBA.portable.IDLEntity They are special because they have a
     * hard-coded typedef defined in the spec.
     *
    ...
    /**
     * Collect and filter thrown exceptions for a given pre-filtered method. Keep only 'checked' exception classes minus
    ...
    protected Vector getMethods(CompoundType ct) {
        ...
        nextMethod: for (int i1 = 0; i1 < mths.length; i1++) { // forall methods
            ...
            if (ctType == TYPE_VALUE) {
                ...
                if ("readObject".equals(mthName) || "writeObject".equals(mthName) || "readExternal".equals(mthName) || "writeExternal".equals(mthName))
                    continue nextMethod; // ignore this one
            }
            if ((ctType == TYPE_NC_CLASS || ctType == TYPE_NC_INTERFACE) && mths[i1].isConstructor()) // init not valid for abstract valuetype
                continue nextMethod; // ignore this one
    ...
    /**
     * Collect constants for a type. A valid constant is a "public final static" field with a compile-time constant value
     * for a primitive type or String
     *
     * @param ct The current CompoundType
    ...
    protected Vector getConstants(CompoundType ct) {
        ...
        for (int i1 = 0; i1 < mems.length; i1++) { // forall members
            ...
            if (mems[i1].isPublic() && mems[i1].isFinal() && mems[i1].isStatic() && (memType.isPrimitive() || "String".equals(memType.getName()))
                    && memValue != null)
    ...
    /**
     * Collect and sort data fields for a ValueType. Sort in Java (not IDL) Unicode name string lexicographic increasing
     * order. Non-static, non-transient fields are mapped. If the type is a custom valuetype, only public fields are mapped.
     *
    ...
    protected Vector getData(CompoundType t) {
        ...
        for (int i1 = 0; i1 < mems.length; i1++) { // forall members
            if (!mems[i1].isStatic() && !mems[i1].isTransient() && (mems[i1].isPublic() || notCust)) {
                int i2;
    ...
    /**
     * Write forward references for referenced interfaces and valuetypes ...but not if the reference is to a boxed
     * IDLEntity,
     *
    ...
    protected void writeForwardReferences(Hashtable refHash, IndentingWriter p) throws IOException {
        ...
        nextReference: while (refEnum.hasMoreElements()) {
    ...
    /**
     * Write forward reference for boxed valuetype for single dimension of IDL sequence. If the dimension is {@literal < 1}
     * and the element is a CompoundType, write a forward declare for the element
     *
    ...
    protected void writeBoxedRMIIncludes(Hashtable arrHash, IndentingWriter p) throws IOException {
        ...
        nextSequence: while (e1.hasMoreElements()) {
    ...
    /**
     * Return the IDL module nesting of the given Type. For IDLEntity CompoundTypes (or their arrays) apply any user
     * specified -idlModule translation or, if none applicable, strip any package prefix. Add boxedIDL or boxedRMI modules
     * if required.
     *
     * @param t Given Type
     * @return Array containing the original module nesting.
    ...
    /**
     * Apply user specified -idlModule translation to package names of given IDLEntity ct. Example: -idlModule foo.bar
     * real::mod::nesting
     *
    ...
    protected boolean translateJavaPackage(CompoundType ct, Vector vec) {
        ...
        if (imHash.size() > 0) { // any -idlModule translation to apply?
            ...
            nextModule: while (k.hasMoreElements()) { // loop thro user-defined -idlModules
    ...
    /**
     * Strip Java #pragma prefix and/or -pkgPrefix prefix package names from given IDLEntity ct. Strip any package prefix
     * which may have been added by comparing with repository id. For example in Java package fake.omega: repid =
     * IDL:phoney.pfix/omega/Juliet:1.0 gives { "omega" }
     *
     * @param ct CompoundType containing given IDLEntity.
    ...
    /**
     * Write boxedRMI valuetype for a single dimension of an IDL sequence indicated by the given OutputType. The filename
     * for the OutputType is of the form "seqn_elemName" where n is the dimension required.
     *
     * @param ot Given OutputType.
     * @param p The output stream.
     */
    protected void writeSequence(OutputType ot, IndentingWriter p) throws IOException {
        ...
        boolean isForwardInclude = et.isCompound() && !isSpecialReference(et) && dim == 1 && !isFromIDL && !"org.omg.CORBA.Object".equals(qName)
    ...
    /**
     * Write inheritance for an IDL interface or valuetype. Any class inheritance precedes any interface inheritance. For a
     * valutype any inheritance from abstract interfaces then follows the "supports" keyword.
     *
     * @param inhHash Hashtable loaded with inherited Types
    ...
    protected void writeInherits(Hashtable inhHash, boolean forValuetype, IndentingWriter p) throws IOException {
        ...
        if (iinh > 0) {
            ...
            while (inhEnum.hasMoreElements()) { // write any interface inheritance
                ct = (CompoundType) inhEnum.nextElement();
                if (!ct.isClass() && !(ct.getTypeCode() == TYPE_ABSTRACT)) {
                    if (i++ > 0)
    ...
    protected void writeData(CompoundType.Member data, IndentingWriter p) throws IOException {
        ...
        if (data.isPublic())
            ...
            p.p("private ");
        p.pln(getQualifiedIDLName(t) + " " + data.getIDLName() + ";");
    }
    ...
    protected void writeBanner(Type t, int dim, boolean isException, IndentingWriter p) throws IOException {
        ...
        String d = DateFormat.getDateTimeInstance(DateFormat.FULL, DateFormat.FULL, Locale.getDefault()).format(new Date());
