...
/**
 * Exception thrown when a parameter passed to a call is out of range or otherwise considered illegal. An ORB may raise
 * this exception if null values or null pointers are passed to an operation (for language mappings where the concept of
 * a null pointers or null values applies). BAD_PARAM can also be raised as a result of a client generating requests
 * with incorrect parameters using the DII.
 * <P>
...
public final class BAD_PARAM extends SystemException {
    ...
    /**
     * Constructs a <code>BAD_PARAM</code> exception with the specified detail message, a minor code of 0, and a completion
    ...
    /**
     * Constructs a <code>BAD_PARAM</code> exception with the specified detail message, minor code, and completion status. A
     * detail message is a <code>String</code> that describes this particular exception.
     *
