...
/**
 * The CORBA <code>TRANSACTION_UNAVAILABLE</code> exception is thrown by the ORB when it cannot process a transaction
 * service context because its connection to the Transaction Service has been abnormally terminated.
 *
 * It contains a minor code, which gives information about what caused the exception, and a completion status. It may
 * also contain a string describing the exception. The OMG CORBA core 2.4 specification has details.
 *
...
public final class TRANSACTION_UNAVAILABLE extends SystemException {
    /**
     * Constructs a <code>TRANSACTION_UNAVAILABLE</code> exception with a default minor code of 0, a completion state of
